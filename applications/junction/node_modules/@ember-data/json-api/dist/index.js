...
function validateHasManyToLinksMode(resourceType, field, _relationshipDoc, _options) {
  throw new Error(`Cannot fetch ${resourceType}.${field.name} because the field is in linksMode but hasMany is not yet supported`);
}
...
class JSONAPICache {
   ...
   */
  patch(op) {
    if (macroCondition(getGlobalConfig().WarpDrive.debug.LOG_OPERATIONS)) {
      try {
   ...
   */
  mutate(mutation) {
    if (macroCondition(getGlobalConfig().WarpDrive.debug.LOG_MUTATIONS)) {
      try {
