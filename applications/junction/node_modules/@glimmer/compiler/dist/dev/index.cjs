    ...
    case HeadKind.Call:
      {
        ...
        let builtExpr = buildCallHead(path, trusted ? wireFormat.VariableResolutionContext.ResolveAsHelperHead : wireFormat.VariableResolutionContext.ResolveAsComponentOrHelperHead, symbols);
...
function buildKeyword(normalized, symbols) {
  ...
  switch (name) {
    case 'let':
      return [wireFormat.SexpOpcodes.Let, util.expect(params, 'let requires params'), block];
    case 'if':
...
function buildVar(head, context, symbols, path) {
  ...
  if (path === undefined || path.length === 0) {
    ...
    util.assert(op !== wireFormat.SexpOpcodes.GetStrictKeyword, '[BUG] keyword with a path');
    return [op, sym, path];
  }
}
...
function expressionContextOp(context) {
  switch (context) {
    ...
    case wireFormat.VariableResolutionContext.ResolveAsComponentOrHelperHead:
      return wireFormat.SexpOpcodes.GetFreeAsComponentOrHelperHead;
    case wireFormat.VariableResolutionContext.ResolveAsHelperHead:
...
class StrictModeValidationPass {
  ...
  Curry(expression) {
    let resolution;
    if (expression.curriedType === vm.CurriedTypes.Component) {
      resolution = ResolutionType.Component;
    } else if (expression.curriedType === vm.CurriedTypes.Helper) {
  ...
  errorFor(name, span, type = ResolutionType.Value) {
    return Err(syntax.generateSyntaxError(`Attempted to resolve a ${type} in a strict mode template, but that value was not in scope: ${name}`, syntax.loc(span)));
  }
}
...
class ExpressionEncoder {
  ...
  Keyword({
    ...
    return [wireFormat.SexpOpcodes.GetStrictKeyword, symbol];
  }
  PathExpression({
    head,
    tail
    ...
    util.assert(getOp[0] !== wireFormat.SexpOpcodes.GetStrictKeyword, '[BUG] keyword in a PathExpression');
