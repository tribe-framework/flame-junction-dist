import { assertNever, dict, expect, isPresentArray, values, assert, NS_XMLNS, NS_XML, NS_XLINK, exhausted, mapPresentArray, getLast, assertPresentArray } from "@glimmer/util";
...
let HeadKind =  function(HeadKind) {
    return HeadKind.Block = "Block", HeadKind.Call = "Call", HeadKind.Element = "Element", 
    HeadKind.AppendPath = "AppendPath", HeadKind.AppendExpr = "AppendExpr", HeadKind.Literal = "Literal", 
    HeadKind.Modifier = "Modifier", HeadKind.DynamicComponent = "DynamicComponent", 
    HeadKind.Comment = "Comment", HeadKind.Splat = "Splat", HeadKind.Keyword = "Keyword", 
    HeadKind;
}({}), VariableKind =  function(VariableKind) {
    return VariableKind.Local = "Local", VariableKind.Free = "Free", VariableKind.Arg = "Arg", 
    VariableKind.Block = "Block", VariableKind.This = "This", VariableKind;
}({});
...
function normalizeBuilderBlockStatement(statement) {
    ...
    blocks = normalizeBlocks(statement[2])) : 4 === statement.length && (params = normalizeParams(statement[1]), 
...
let Builder =  function(Builder) {
    return Builder[Builder.Literal = 0] = "Literal", Builder[Builder.Comment = 1] = "Comment", 
    Builder[Builder.Append = 2] = "Append", Builder[Builder.Modifier = 3] = "Modifier", 
    Builder[Builder.DynamicComponent = 4] = "DynamicComponent", Builder[Builder.Get = 5] = "Get", 
    Builder[Builder.Concat = 6] = "Concat", Builder[Builder.HasBlock = 7] = "HasBlock", 
    Builder[Builder.HasBlockParams = 8] = "HasBlockParams", Builder;
}({}), ExpressionKind =  function(ExpressionKind) {
    return ExpressionKind.Literal = "Literal", ExpressionKind.Call = "Call", ExpressionKind.GetPath = "GetPath", 
    ExpressionKind.GetVar = "GetVar", ExpressionKind.Concat = "Concat", ExpressionKind.HasBlock = "HasBlock", 
    ExpressionKind.HasBlockParams = "HasBlockParams", ExpressionKind;
}({});
    ...
    switch (normalized.kind) {
      ...
      case HeadKind.Element:
        return function({name: name, attrs: attrs, block: block}, symbols) {
            ...
            if (attrs) {
                let {params: params, args: args} = function(attrs, symbols) {
                    let params = [], keys = [], values = [];
                    for (const [key, value] of Object.entries(attrs)) value === HeadKind.Splat ? params.push([ SexpOpcodes.AttrSplat, symbols.block("&attrs") ]) : "@" === key[0] ? (keys.push(key), 
                    values.push(buildExpression(value, "Strict", symbols))) : params.push(...buildAttributeValue(key, value, 
                ...
                out.push(...params), assert(null === args, "Can't pass args to a simple element");
            }
            if (out.push([ SexpOpcodes.FlushElement ]), Array.isArray(block)) block.forEach((s => out.push(...buildStatement(s, symbols)))); else if (null !== block) throw assertNever(block);
            return out.push([ SexpOpcodes.CloseElement ]), out;
        }(normalized, symbols);
...
function buildKeyword(normalized, symbols) {
    ...
    switch (name) {
      case "let":
        return [ SexpOpcodes.Let, expect(params, "let requires params"), block ];

      case "if":
        return [ SexpOpcodes.If, expect(params, "if requires params")[0], block, inverse ];

      case "each":
        {
            let keyExpr = normalized.hash ? normalized.hash.key : null, key = keyExpr ? buildExpression(keyExpr, "Strict", symbols) : null;
            return [ SexpOpcodes.Each, expect(params, "if requires params")[0], key, block, inverse ];
        }
...
function buildVar(head, context, symbols, path) {
    ...
    }(head.kind, symbols, head.name)), void 0 === path || 0 === path.length ? [ op, sym ] : (assert(op !== SexpOpcodes.GetStrictKeyword, "[BUG] keyword with a path"), 
    [ op, sym, path ]);
}
...
function assertCurryKeyword(curriedType) {
    return (node, state) => {
        let readableType = CurriedTypeToReadableType[curriedType], stringsAllowed = curriedType === CurriedTypes.Component, {args: args} = node, definition = args.nth(0);
...
const logKeyword = {
    ...
    translate: ({node: node, state: {scope: scope}}) => (scope.setHasDebugger(), Ok(new Debugger({
        loc: node.loc,
        scope: scope
    })))
}).kw("component", {
...
var ResolutionType =  function(ResolutionType) {
    return ResolutionType.Value = "value", ResolutionType.Component = "component", ResolutionType.Helper = "helper", 
    ResolutionType.Modifier = "modifier", ResolutionType.ComponentOrHelper = "component or helper", 
    ResolutionType;
}(ResolutionType || {});
...
class StrictModeValidationPass {
    ...
    Curry(expression) {
        let resolution;
        return resolution = expression.curriedType === CurriedTypes.Component ? ResolutionType.Component : expression.curriedType === CurriedTypes.Helper ? ResolutionType.Helper : ResolutionType.Modifier, 
...
const EXPR = new class {
    ...
    PathExpression({head: head, tail: tail}) {
        ...
        return assert(getOp[0] !== SexpOpcodes.GetStrictKeyword, "[BUG] keyword in a PathExpression"), 
        [ ...getOp, EXPR.Tail(tail) ];
    }
...
function precompile(source, options = defaultOptions) {
    const [block, usedLocals] = precompileJSON(source, options), moduleName = options.meta?.moduleName, idFn = options.id || defaultId, blockJSON = JSON.stringify(block), templateJSONObject = {
...
export { Builder, NEWLINE, ProgramSymbols, WireFormatDebugger, buildStatement, buildStatements, c, defaultId, precompile, precompileJSON, s, unicode };
