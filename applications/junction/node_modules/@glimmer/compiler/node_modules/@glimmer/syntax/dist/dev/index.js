...
var Char =  function(Char) {
    return Char[Char.NBSP = 160] = "NBSP", Char[Char.QUOT = 34] = "QUOT", Char[Char.LT = 60] = "LT", 
    Char[Char.GT = 62] = "GT", Char[Char.AMP = 38] = "AMP", Char;
}(Char || {});

const ATTR_VALUE_REGEX_TEST = /["&\xA0]/u, ATTR_VALUE_REGEX_REPLACE = new RegExp(ATTR_VALUE_REGEX_TEST.source, "gu"), TEXT_REGEX_TEST = /[&<>\xA0]/u, TEXT_REGEX_REPLACE = new RegExp(TEXT_REGEX_TEST.source, "gu");
...
let OffsetKind =  function(OffsetKind) {
    return OffsetKind.CharPosition = "CharPosition", OffsetKind.HbsPosition = "HbsPosition", 
    OffsetKind.InternalsSynthetic = "InternalsSynthetic", OffsetKind.NonExistent = "NonExistent", 
    OffsetKind.Broken = "Broken", OffsetKind;
}({});
...
class Matcher {
   ...
   */
    ...
    matchFor(left, right) {
        ...
        return assert(null !== callback, `no match defined for (${left}, ${right}) and no AnyMatch defined either`), 
        callback;
    }
        ...
        for (;seenChars < sourceLength; ) {
            ...
            if (-1 === nextLine && (nextLine = this.source.length), seenLines === line - 1) {
                ...
                if (DEBUG) {
                    ...
                    assert(null !== roundTrip, "the returned offset failed to round-trip"), assert(roundTrip.line === line, "the round-tripped line didn't match the original line"), 
...
const visitorKeys = {
    ...
    return TraversalError.prototype = Object.create(Error.prototype), TraversalError.prototype.constructor = TraversalError, 
    TraversalError;
}();
...
function visitNode(visitor, path) {
    let enter, exit, result, {node: node, parent: parent, parentKey: parentKey} = path, handler = function(visitor, nodeType) {
        if (visitor.Program && ("Template" === nodeType && !visitor.Template || "Block" === nodeType && !visitor.Block)) return deprecate(`The 'Program' visitor node is deprecated. Use 'Template' or 'Block' instead (node was '${nodeType}') `), 
        visitor.Program;
...
function visitArray(visitor, array, parent, parentKey) {
    for (let i = 0; i < array.length; i++) {
        let node = unwrap(array[i]), result = visitNode(visitor, new WalkerPath(node, parent, parentKey));
...
function buildPath(path, loc) {
    ...
    if ("string" != typeof path) {
        ...
        {
            assert(-1 === path.head.indexOf("."), "builder.path({ head, tail }) should not be called with a head with dots in it");
    ...
    let {head: head, tail: tail} = function(original, loc) {
        let [head, ...tail] = asPresentArray(original.split(".")), headNode = b.head({
            original: head,
            loc: buildLoc(loc || null)
        });
        return b.path({
...
var publicBuilder = {
    ...
    block: function(path, params, hash, _defaultBlock, _elseBlock = null, loc, openStrip, inverseStrip, closeStrip) {
        ...
        return "Template" === _defaultBlock.type ? (deprecate("b.program is deprecated. Use b.blockItself instead."), 
        defaultBlock = b.blockItself({
            params: buildBlockParams(_defaultBlock.blockParams),
            body: _defaultBlock.body,
            loc: _defaultBlock.loc
        })) : defaultBlock = _defaultBlock, "Template" === _elseBlock?.type ? (deprecate("b.program is deprecated. Use b.blockItself instead."), 
        assert(0 === _elseBlock.blockParams.length, "{{else}} block cannot have block params"), 
    ...
    function(tag, options = {}) {
        ...
        selfClosing = !0) : path = buildPath(tag) : "type" in tag ? (assert("PathExpression" === tag.type, `Invalid tag type ${tag.type}`), 
        path = tag) : "path" in tag ? (assert("PathExpression" === tag.path.type, `Invalid tag type ${tag.path.type}`), 
        path = tag.path, selfClosing = tag.selfClosing) : (path = buildPath(tag.name), selfClosing = tag.selfClosing), 
        selfClosing && assert(null == _closeTag, "Cannot build a self-closing tag with a closeTag source location");
        let params = blockParams?.map((param => "string" == typeof param ? buildVar(param) : param)), closeTag = null;
        return _closeTag ? closeTag = buildLoc(_closeTag || null) : void 0 === _closeTag && (closeTag = selfClosing || isVoidTag(path.original) ? null : buildLoc(null)), 
        b.element({
            path: path,
    ...
    program: function(body, blockParams, loc) {
        return deprecate("b.program is deprecated. Use b.template or b.blockItself instead."), 
        blockParams && blockParams.length ? buildBlockItself(body, blockParams, !1, loc) : buildTemplate(body, [], loc);
    },
...
class {
    ...
    blockItself({body: body, params: params, chained: chained = !1, loc: loc}) {
        return {
            type: "Block",
            body: body,
            params: params,
    ...
    mustache({path: path, params: params, hash: hash, trusting: trusting, loc: loc, strip: strip = DEFAULT_STRIP}) {
        return function({path: path, params: params, hash: hash, trusting: trusting, strip: strip, loc: loc}) {
            ...
            return Object.defineProperty(node, "escaped", {
                enumerable: !1,
                get() {
                    return deprecate("The escaped property on mustache nodes is deprecated, use trusting instead"), 
                    !this.trusting;
                },
                set(value) {
    ...
    block({path: path, params: params, hash: hash, defaultBlock: defaultBlock, elseBlock: elseBlock = null, loc: loc, openStrip: openStrip = DEFAULT_STRIP, inverseStrip: inverseStrip = DEFAULT_STRIP, closeStrip: closeStrip = DEFAULT_STRIP}) {
        return {
            type: "BlockStatement",
            path: path,
            params: params,
    ...
    mustacheComment({value: value, loc: loc}) {
        return {
    ...
    element({path: path, selfClosing: selfClosing, attributes: attributes, modifiers: modifiers, params: params, comments: comments, children: children, openTag: openTag, closeTag: closeTag, loc: loc}) {
    ...
    elementModifier({path: path, params: params, hash: hash, loc: loc}) {
        return {
    ...
    attr({name: name, value: value, loc: loc}) {
        return {
            type: "AttrNode",
            name: name,
            value: value,
    ...
    sexpr({path: path, params: params, hash: hash, loc: loc}) {
        return {
            type: "SubExpression",
            path: path,
            params: params,
    ...
    path({head: head, tail: tail, loc: loc}) {
        return function({head: head, tail: tail, loc: loc}) {
            ...
            return Object.defineProperty(node, "parts", {
                ...
                enumerable: !1,
                get() {
                    return deprecate("The this property on path nodes is deprecated, use head.type instead"), 
                    "ThisHead" === this.head.type;
                }
            }), Object.defineProperty(node, "data", {
                enumerable: !1,
                get() {
                    return deprecate("The data property on path nodes is deprecated, use head.type instead"), 
                    "AtHead" === this.head.type;
                }
            }), node;
        }({
            head: head,
    ...
    atName({name: name, loc: loc}) {
        ...
        const node = {
            ...
            set name(value) {
                ...
                assert(-1 === value.indexOf("."), "builder.at() should not be called with a name with dots in it"), 
                _name = value;
            },
            get original() {
                return this.name;
    ...
    var({name: name, loc: loc}) {
        ...
        const node = {
            ...
            set name(value) {
                assert("this" !== value, "You called builders.var() with 'this'. Call builders.this instead"), 
                assert("@" !== value[0], `You called builders.var() with '${name}'. Call builders.at('${name}') instead`), 
                assert(-1 === value.indexOf("."), "builder.var() should not be called with a name with dots in it"), 
                _name = value;
            },
            get original() {
                return this.name;
    ...
    pair({key: key, value: value, loc: loc}) {
        return {
            type: "HashPair",
            key: key,
            value: value,
    ...
    literal({type: type, value: value, loc: loc}) {
        return function({type: type, value: value, loc: loc}) {
            const node = {
                type: type,
                value: value,
                loc: loc
            ...
            return Object.defineProperty(node, "original", {
                enumerable: !1,
                get() {
                    return deprecate("The original property on literal nodes is deprecated, use value instead"), 
                    this.value;
                },
                set(value) {
...
class Parser {
    ...
    get currentTag() {
        ...
        return assert(node && ("StartTag" === node.type || "EndTag" === node.type), "expected tag"), 
        node;
    }
    get currentStartTag() {
        let node = this.currentNode;
        return assert(node && "StartTag" === node.type, "expected start tag"), node;
    }
    get currentEndTag() {
        let node = this.currentNode;
        return assert(node && "EndTag" === node.type, "expected end tag"), node;
    }
    get currentComment() {
        let node = this.currentNode;
        return assert(node && "CommentStatement" === node.type, "expected a comment"), node;
    }
    get currentData() {
        let node = this.currentNode;
        return assert(node && "TextNode" === node.type, "expected a text node"), node;
    }
    acceptNode(node) {
    ...
    sourceForNode(node, endNode) {
        ...
        lastColumn = node.loc.end.column); currentLine < lastLine; ) currentLine++, line = unwrap(this.lines[currentLine]), 
...
class HandlebarsNodeVisitors extends Parser {
    ...
    parseProgram(node, program) {
        ...
        // Ensure that that the element stack is balanced properly.
            ...
            assert(void 0 !== poppedNode, "[BUG] empty parser elementStack"), assert(!1, `[BUG] mismatched parser elementStack node: ${node.type}`);
        }
        return node;
    }
    BlockStatement(block) {
        ...
        const program = this.Program(block.program, blockParams), inverse = block.inverse ? this.Program(block.inverse, []) : null, node = b.block({
            path: path,
            params: params,
            hash: hash,
...
function acceptCallNodes(compiler, node) {
    ...
    const params = node.params ? node.params.map((e => compiler.acceptNode(e))) : [], end = isPresentArray(params) ? getLast(params).loc : path.loc;
...
class TokenizerEventHandlers extends HandlebarsNodeVisitors {
    ...
    finishStartTag() {
        let {name: name, nameStart: nameStart, nameEnd: nameEnd} = this.currentStartTag;
        // <> should probably be a syntax error, but s-h-t is currently broken for that case
                assert("" !== name, "tag name cannot be empty"), assert(null !== nameStart, "nameStart unexpectedly null"), 
        assert(null !== nameEnd, "nameEnd unexpectedly null");
        let nameLoc = nameStart.until(nameEnd), [head, ...tail] = asPresentArray(name.split(".")), path = b.path({
            head: b.head({
                original: head,
    ...
    finishEndTag(isVoid) {
        ...
        isVoid ? element.closeTag = null : element.selfClosing ? assert(null === element.closeTag, "element.closeTag unexpectedly present") : element.closeTag = closeTagStart.until(this.offset()), 
    ...
    appendToTagName(char) {
        ...
        if (tag.name += char, "StartTag" === tag.type) {
            ...
            null === tag.nameStart && (assert(null === tag.nameEnd, "nameStart and nameEnd must both be null"), 
    ...
    parsePossibleBlockParams() {
        ...
        assert("attributeName" === this.tokenizer.state, "must be in TokenizerState.attributeName");
        ...
        const handlers = {
            PossibleAs: next => {
                if (assert("PossibleAs" === state.state, "bug in block params parser"), isSpace(next)) 
                // " as ..."
                state = {
            ...
            BeforeStartPipe: next => {
                assert("BeforeStartPipe" === state.state, "bug in block params parser"), isSpace(next) ? this.tokenizer.consume() : "|" === next ? (state = {
            ...
            BeforeBlockParamName: next => {
                if (assert("BeforeBlockParamName" === state.state, "bug in block params parser"), 
                isSpace(next)) this.tokenizer.consume(); else if ("" === next) 
            ...
            BlockParamName: next => {
                if (assert("BlockParamName" === state.state, "bug in block params parser"), "" === next) 
                // The HTML tokenizer ran out of characters, so we are either
            ...
            AfterEndPipe: next => {
                assert("AfterEndPipe" === state.state, "bug in block params parser"), isSpace(next) ? this.tokenizer.consume() : "" === next ? (
                // The HTML tokenizer ran out of characters, so we are either
                // encountering mustache or <EOF>, HBS side will attach the error
                // to the next span
                state = {
            ...
            Error: next => {
                if (assert("Error" === state.state, "bug in block params parser"), "" === next || "/" === next || ">" === next || isSpace(next)) throw generateSyntaxError(state.message, state.start.until(this.offset()));
                // Slurp up the next "token" for the error span
                this.tokenizer.consume();
            },
            Done: () => {
                assert(!1, "This should never be called");
            }
        };
        let next;
        do {
        ...
        assert("Done" === state.state, "bug in block params parser");
    }
...
let FreeVarNamespace =  function(FreeVarNamespace) {
    return FreeVarNamespace.Helper = "Helper", FreeVarNamespace.Modifier = "Modifier", 
    FreeVarNamespace.Component = "Component", FreeVarNamespace;
}({});

const HELPER_NAMESPACE = FreeVarNamespace.Helper, MODIFIER_NAMESPACE = FreeVarNamespace.Modifier, COMPONENT_NAMESPACE = FreeVarNamespace.Component;
...
class Builder {
    ...
    at(name, symbol, loc) {
        ...
        return assert("@" === name[0], "call builders.at() with a string that starts with '@'"), 
        new ArgReference({
            loc: loc,
    ...
    freeVar({name: name, context: context, symbol: symbol, loc: loc}) {
        return assert("this" !== name, "You called builders.freeVar() with 'this'. Call builders.this instead"), 
        assert("@" !== name[0], `You called builders.freeVar() with '${name}'. Call builders.at('${name}') instead`), 
        new FreeVarReference({
            name: name,
            resolution: context,
            symbol: symbol,
            loc: loc
    ...
    localVar(name, symbol, isTemplateLocal, loc) {
        return assert("this" !== name, "You called builders.var() with 'this'. Call builders.this instead"), 
 ...
 */ class ExpressionNormalizer {
   ...
   */    normalize(expr, resolution) {
        switch (expr.type) {
          ...
          case "PathExpression":
            return assert(resolution, "[BUG] resolution is required"), this.path(expr, resolution);
 ...
 */ class StatementNormalizer {
    ...
    MustacheCommentStatement(node) {
        let textLoc, loc = this.block.loc(node.loc);
...
class ElementNormalizer {
   ...
   */    ElementNode(element) {
        let {tag: tag, selfClosing: selfClosing, comments: comments} = element, loc = this.ctx.loc(element.loc), [tagHead, ...rest] = asPresentArray(tag.split(".")), path = this.classifyTag(tagHead, rest, element.loc), attrs = element.attributes.filter((a => "@" !== a.name[0])).map((a => this.attr(a))), args = element.attributes.filter((a => "@" === a.name[0])).map((a => this.arg(a))), modifiers = element.modifiers.map((m => this.modifier(m))), child = this.ctx.child(element.blockParams), normalizer = new StatementNormalizer(child), childNodes = element.children.map((s => normalizer.normalize(s))), el = this.ctx.builder.element({
            selfClosing: selfClosing,
            attrs: attrs,
            componentArgs: args,
            modifiers: modifiers,
   ...
   */    attrPart(part) {
    ...
    attr(m) {
        if (assert("@" !== m.name[0], "An attr name must not start with `@`"), "...attributes" === m.name) return this.ctx.builder.splatAttr(this.ctx.table.allocateBlock("attrs"), this.ctx.loc(m.loc));
    ...
    arg(arg) {
        assert("@" === arg.name[0], "An arg name must start with `@`"), this.checkArgCall(arg);
