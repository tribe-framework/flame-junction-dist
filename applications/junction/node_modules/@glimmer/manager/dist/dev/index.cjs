...
const FROM_CAPABILITIES = env.DEBUG ? new WeakSet() : undefined;
...
const EMPTY = vm.InternalComponentCapabilities.Empty;
/**
...
function capability(capabilities, capability) {
  return capabilities[capability] ? vm.InternalComponentCapabilities[capability] : EMPTY;
}
...
class CustomHelperManager {
  ...
  getHelper(definition) {
    return (capturedArgs, owner) => {
      ...
      if (hasValue(manager)) {
        let cache = reference.createComputeRef(() => manager.getValue(bucket), null, env.DEBUG && manager.getDebugName && manager.getDebugName(definition));
        if (hasDestroyable(manager)) {
          destroyable.associateDestroyableChild(cache, manager.getDestroyable(bucket));
        }
        return cache;
      } else if (hasDestroyable(manager)) {
        let ref = reference.createConstRef(undefined, env.DEBUG && (manager.getDebugName?.(definition) ?? 'unknown helper'));
...
class CustomComponentManager {
  ...
  getDestroyable(bucket) {
    ...
    if (hasDestructors(delegate)) {
      ...
      destroyable.registerDestructor(bucket, () => delegate.destroyComponent(component));
      return bucket;
    }
    return null;
  }
...
class CustomModifierManager {
  ...
  create(owner, element, definition, capturedArgs) {
    ...
    let tag = validator.createUpdatableTag();
    let state;
    state = {
      tag,
      element,
    ...
    destroyable.registerDestructor(state, () => delegate.destroyModifier(instance, args));
    return state;
  }
  getDebugName(definition) {
  ...
  install({
    ...
    if (capabilities.disableAutoTracking === true) {
      validator.untrack(() => delegate.installModifier(modifier, util.castToBrowser(element, 'ELEMENT'), args));
    } else {
      delegate.installModifier(modifier, util.castToBrowser(element, 'ELEMENT'), args);
    }
  }
  update({
    args,
    ...
    if (capabilities.disableAutoTracking === true) {
      validator.untrack(() => delegate.updateModifier(modifier, args));
    } else {
