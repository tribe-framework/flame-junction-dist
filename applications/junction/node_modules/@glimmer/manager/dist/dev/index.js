...
import { Op, InternalComponentCapabilities } from "@glimmer/vm";
...
class NamedArgsProxy {
    ...
    getOwnPropertyDescriptor(_target, prop) {
        if (DEBUG && !(prop in this.named)) throw new Error(`args proxies do not have real property descriptors, so you should never need to call getOwnPropertyDescriptor yourself. This code exists for enumerability, such as in for-in loops and Object.keys(). Attempted to get the descriptor for \`${String(prop)}\``);
        return {
            enumerable: !0,
            configurable: !0
        };
...
class PositionalArgsProxy {
    ...
    get(target, prop) {
        let {positional: positional} = this;
...
const argsProxyFor = (capturedArgs, type) => {
    const {named: named, positional: positional} = capturedArgs, namedHandler = new NamedArgsProxy(named), positionalHandler = new PositionalArgsProxy(positional), namedTarget = Object.create(null);
    if (DEBUG) {
...
const FROM_CAPABILITIES = DEBUG ? new WeakSet : void 0;

function buildCapabilities(capabilities) {
    return DEBUG && (FROM_CAPABILITIES.add(capabilities), Object.freeze(capabilities)), 
    capabilities;
}
...
function helperCapabilities(managerAPI, options = {}) {
    if (DEBUG && "3.23" !== managerAPI) throw new Error("Invalid helper manager compatibility specified");
    if (DEBUG && (!options.hasValue && !options.hasScheduledEffect || options.hasValue && options.hasScheduledEffect)) throw new Error("You must pass either the `hasValue` OR the `hasScheduledEffect` capability when defining a helper manager. Passing neither, or both, is not permitted.");
...
class CustomHelperManager {
    ...
    getDelegateFor(owner) {
        if (void 0 === owner) {
            let {undefinedDelegate: undefinedDelegate} = this;
    ...
    getHelper(definition) {
        return (capturedArgs, owner) => {
            ...
            if (hasDestroyable(manager)) {
                let ref = createConstRef(void 0, DEBUG && (manager.getDebugName?.(definition) ?? "unknown helper"));
...
const COMPONENT_MANAGERS = new WeakMap, MODIFIER_MANAGERS = new WeakMap, HELPER_MANAGERS = new WeakMap, getPrototypeOf$1 = Object.getPrototypeOf;

function setManager(map, manager, obj) {
    if (DEBUG && ("object" != typeof obj || null === obj) && "function" != typeof obj) throw new Error(`Attempted to set a manager on a non-object value. Managers can only be associated with objects or functions. Value was ${debugToString(obj)}`);
    if (DEBUG && map.has(obj)) throw new Error(`Attempted to set the same type of manager multiple times on a value. You can only associate one manager of each type with a given value. Value was ${debugToString(obj)}`);
...
function getInternalModifierManager(definition, isOptional) {
    if (DEBUG && "function" != typeof definition && ("object" != typeof definition || null === definition)) throw new Error(`Attempted to use a value as a modifier, but it was not an object or function. Modifier definitions must be objects or functions with an associated modifier manager. The value was: ${definition}`);
    const manager = getManager(MODIFIER_MANAGERS, definition);
    if (void 0 === manager) {
        if (!0 === isOptional) return null;
        if (DEBUG) throw new Error(`Attempted to load a modifier, but there wasn't a modifier manager associated with the definition. The definition was: ${debugToString(definition)}`);
    }
    return manager;
}
...
function getInternalHelperManager(definition, isOptional) {
    if (DEBUG && "function" != typeof definition && ("object" != typeof definition || null === definition)) throw new Error(`Attempted to use a value as a helper, but it was not an object or function. Helper definitions must be objects or functions with an associated helper manager. The value was: ${definition}`);
    ...
    if (!0 === isOptional) return null;
    if (DEBUG) throw new Error(`Attempted to load a helper, but there wasn't a helper manager associated with the definition. The definition was: ${debugToString(definition)}`);
    return null;
}
...
function getInternalComponentManager(definition, isOptional) {
    if (DEBUG && "function" != typeof definition && ("object" != typeof definition || null === definition)) throw new Error(`Attempted to use a value as a component, but it was not an object or function. Component definitions must be objects or functions with an associated component manager. The value was: ${definition}`);
    const manager = getManager(COMPONENT_MANAGERS, definition);
    if (void 0 === manager) {
        if (!0 === isOptional) return null;
        if (DEBUG) throw new Error(`Attempted to load a component, but there wasn't a component manager associated with the definition. The definition was: ${debugToString(definition)}`);
    }
    return manager;
}
...
function componentCapabilities(managerAPI, options = {}) {
    if (DEBUG && "3.13" !== managerAPI) throw new Error("Invalid component manager compatibility specified");
...
class CustomComponentManager {
    ...
    getDelegateFor(owner) {
        let {componentManagerDelegates: componentManagerDelegates} = this, delegate = componentManagerDelegates.get(owner);
        if (void 0 === delegate) {
            let {factory: factory} = this;
            if (delegate = factory(owner), DEBUG && !FROM_CAPABILITIES.has(delegate.capabilities)) 
            // TODO: This error message should make sense in both Ember and Glimmer https://github.com/glimmerjs/glimmer-vm/issues/1200
    ...
    update(bucket) {
        let {delegate: delegate} = bucket;
        if (hasUpdateHook(delegate)) {
            let {component: component, args: args} = bucket;
            delegate.updateComponent(component, args);
        }
    }
    didCreate({component: component, delegate: delegate}) {
        hasAsyncLifeCycleCallbacks(delegate) && delegate.didCreateComponent(component);
    }
    didUpdate({component: component, delegate: delegate}) {
        (function(delegate) {
    ...
    didRenderLayout() {}
    didUpdateLayout() {}
    getSelf({component: component, delegate: delegate}) {
        return createConstRef(delegate.getContext(component), "this");
    }
    getDestroyable(bucket) {
        const {delegate: delegate} = bucket;
...
function modifierCapabilities(managerAPI, optionalFeatures = {}) {
    if (DEBUG && "3.22" !== managerAPI) throw new Error("Invalid modifier manager compatibility specified");
    return buildCapabilities({
...
*/ class CustomModifierManager {
    ...
    getDebugInstance({modifier: modifier}) {
        return modifier;
    }
    getTag({tag: tag}) {
        return tag;
    }
    install({element: element, args: args, modifier: modifier, delegate: delegate}) {
        let {capabilities: capabilities} = delegate;
    ...
    update({args: args, modifier: modifier, delegate: delegate}) {
        let {capabilities: capabilities} = delegate;
...
const TEMPLATES = new WeakMap, getPrototypeOf = Object.getPrototypeOf;

function setComponentTemplate(factory, obj) {
    if (DEBUG && (null === obj || "object" != typeof obj && "function" != typeof obj)) throw new Error(`Cannot call \`setComponentTemplate\` on \`${debugToString(obj)}\``);
    if (DEBUG && TEMPLATES.has(obj)) throw new Error(`Cannot call \`setComponentTemplate\` multiple times on the same class (\`${debugToString(obj)}\`)`);
