...
var encoder = require('@glimmer/encoder');
...
function makeResolutionTypeVerifier(typeToVerify) {
  return opcode => {
    ...
    let type = opcode[0];
    return type === wireFormat.SexpOpcodes.GetStrictKeyword || type === wireFormat.SexpOpcodes.GetLexicalSymbol || type === typeToVerify;
  };
}
const isGetFreeComponent = makeResolutionTypeVerifier(wireFormat.SexpOpcodes.GetFreeAsComponentHead);
const isGetFreeModifier = makeResolutionTypeVerifier(wireFormat.SexpOpcodes.GetFreeAsModifierHead);
const isGetFreeHelper = makeResolutionTypeVerifier(wireFormat.SexpOpcodes.GetFreeAsHelperHead);
const isGetFreeComponentOrHelper = makeResolutionTypeVerifier(wireFormat.SexpOpcodes.GetFreeAsComponentOrHelperHead);
...
function resolveComponent(resolver, constants, meta, [, expr, then]) {
  util.assert(isGetFreeComponent(expr), 'Attempted to resolve a component with incorrect opcode');
  let type = expr[0];
  if (env.DEBUG && expr[0] === wireFormat.SexpOpcodes.GetStrictKeyword) {
    util.assert(!meta.isStrictMode, 'Strict mode errors should already be handled at compile time');
  ...
  if (type === wireFormat.SexpOpcodes.GetLexicalSymbol) {
    let {
      scopeValues,
      owner
    } = meta;
    let definition = util.expect(scopeValues, 'BUG: scopeValues must exist if template symbol is used')[expr[1]];
    then(constants.component(definition, util.expect(owner, 'BUG: expected owner when resolving component definition')));
  } else {
    let {
      upvars,
      owner
    } = assertResolverInvariants(meta);
    let name = util.unwrap(upvars[expr[1]]);
    let definition = resolver.lookupComponent(name, owner);
    if (env.DEBUG && (typeof definition !== 'object' || definition === null)) {
      util.assert(!meta.isStrictMode, 'Strict mode errors should already be handled at compile time');
...
function resolveHelper(resolver, constants, meta, [, expr, then]) {
  util.assert(isGetFreeHelper(expr), 'Attempted to resolve a helper with incorrect opcode');
  let type = expr[0];
  if (type === wireFormat.SexpOpcodes.GetLexicalSymbol) {
    let {
      scopeValues
  ...
  } else if (type === wireFormat.SexpOpcodes.GetStrictKeyword) {
...
function resolveModifier(resolver, constants, meta, [, expr, then]) {
  util.assert(isGetFreeModifier(expr), 'Attempted to resolve a modifier with incorrect opcode');
  let type = expr[0];
  if (type === wireFormat.SexpOpcodes.GetLexicalSymbol) {
    let {
      scopeValues
...
function resolveComponentOrHelper(resolver, constants, meta, [, expr, {
  ...
  util.assert(isGetFreeComponentOrHelper(expr), 'Attempted to resolve a component or helper with incorrect opcode');
  let type = expr[0];
  if (type === wireFormat.SexpOpcodes.GetLexicalSymbol) {
    let {
      scopeValues,
    ...
    let definition = util.expect(scopeValues, 'BUG: scopeValues must exist if template symbol is used')[expr[1]];
    let component = constants.component(definition, util.expect(owner, 'BUG: expected owner when resolving component definition'), true);
    if (component !== null) {
    ...
    ifHelper(util.expect(helper, 'BUG: helper must exist'));
  } else if (type === wireFormat.SexpOpcodes.GetStrictKeyword) {
...
function resolveOptionalComponentOrHelper(resolver, constants, meta, [, expr, {
  ...
  util.assert(isGetFreeComponentOrHelper(expr), 'Attempted to resolve an optional component or helper with incorrect opcode');
  let type = expr[0];
  if (type === wireFormat.SexpOpcodes.GetLexicalSymbol) {
    let {
      scopeValues,
...
function nonSmallIntOperand(value) {
  util.assert(!util.isSmallInt(value), 'Attempted to make a operand for an int that was not a small int, you should encode this as an immediate');
  return {
...
class Labels {
  ...
  patch(heap) {
    ...
    for (const {
      ...
      util.assert(heap.getbyaddr(at) === -1, 'Expected heap to contain a placeholder, but it did not');
  ...
  if (isBuilderOpcode(op[0])) {
    ...
    switch (op[0]) {
      ...
      case HighLevelResolutionOpcodes.Local:
        {
          let freeVar = op[1];
          let name = util.expect(meta.upvars, 'BUG: attempted to resolve value but no upvars found')[freeVar];
          let andThen = op[2];
      ...
      case HighLevelResolutionOpcodes.TemplateLocal:
        {
          ...
          let value = util.expect(meta.scopeValues, 'BUG: Attempted to get a template local, but template does not have any')[valueIndex];
...
class EncoderImpl {
  labelsStack = new util.Stack();
  encoder = new encoder.InstructionEncoderImpl([]);
  errors = [];
  handle;
  ...
  error(error) {
    this.encoder.encode(vm.Op.Primitive, 0);
    this.errors.push(error);
  }
  commit(size) {
    let handle = this.handle;
    this.heap.pushMachine(vm.MachineOp.Return);
    this.heap.finishMalloc(handle, size);
    if (util.isPresentArray(this.errors)) {
      return {
        errors: this.errors,
        handle
      };
  ...
  push(constants, type, ...args) {
    ...
    if (env.DEBUG && type > vm.TYPE_SIZE) {
      throw new Error(`Opcode type over 8-bits. Got ${type}.`);
    }
    let machine = vm.isMachineOp(type) ? vm.MACHINE_MASK : 0;
    let first = type | machine | args.length << vm.ARG_SHIFT;
    heap.pushRaw(first);
  ...
  operand(constants, operand) {
    ...
    if (typeof operand === 'object' && operand !== null) {
      if (Array.isArray(operand)) {
        return util.encodeHandle(constants.array(operand));
      } else {
        switch (operand.type) {
          ...
          case HighLevelOperands.Block:
            return util.encodeHandle(constants.value(compilableBlock(operand.value, this.meta)));
          case HighLevelOperands.StdLib:
            return util.expect(this.stdlib, 'attempted to encode a stdlib operand, but the encoder did not have a stdlib. Are you currently building the stdlib?')[operand.value];
    ...
    return util.encodeHandle(constants.value(operand));
  }
  get currentLabels() {
    return util.expect(this.labelsStack.current, 'bug: not in a label stack');
  }
  label(name) {
  ...
  stopLabels() {
    let label = util.expect(this.labelsStack.pop(), 'unbalanced push and pop labels');
...
class NamedBlocksImpl {
  ...
  with(name, block) {
    ...
    if (blocks) {
      return new NamedBlocksImpl(util.assign({}, blocks, {
        [name]: block
      }));
    } else {
...
function namedBlocks(blocks) {
  ...
  for (const [i, key] of util.enumerate(keys)) {
    out[key] = util.unwrap(values[i]);
  }
...
function Call(op, handle, positional, named) {
  op(vm.MachineOp.PushFrame);
  SimpleArgs(op, positional, named, false);
  op(vm.Op.Helper, handle);
  op(vm.MachineOp.PopFrame);
  op(vm.Op.Fetch, vm.$v0);
}
...
function CallDynamic(op, positional, named, append) {
  op(vm.MachineOp.PushFrame);
  SimpleArgs(op, positional, named, false);
  op(vm.Op.Dup, vm.$fp, 1);
  op(vm.Op.DynamicHelper);
  if (append) {
    op(vm.Op.Fetch, vm.$v0);
    append();
    op(vm.MachineOp.PopFrame);
...
function DynamicScope(op, names, block) {
  op(vm.Op.PushDynamicScope);
  op(vm.Op.BindDynamicScope, names);
  block();
  op(vm.Op.PopDynamicScope);
}
function Curry(op, type, definition, positional, named) {
  op(vm.MachineOp.PushFrame);
  SimpleArgs(op, positional, named, false);
  op(vm.Op.CaptureArgs);
  expr(op, definition);
  op(vm.Op.Curry, type, isStrictMode());
  op(vm.MachineOp.PopFrame);
  op(vm.Op.Fetch, vm.$v0);
}
...
class Compilers {
  ...
  compile(op, sexp) {
    let name = sexp[0];
    let index = util.unwrap(this.names[name]);
    let func = this.funcs[index];
    util.assert(!!func, `expected an implementation for ${sexp[0]}`);
    func(op, sexp);
  }
}

const EXPRESSIONS = new Compilers();
EXPRESSIONS.add(wireFormat.SexpOpcodes.Concat, (op, [, parts]) => {
  for (let part of parts) {
    expr(op, part);
  }
  op(vm.Op.Concat, parts.length);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.Call, (op, [, expression, positional, named]) => {
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.Curry, (op, [, expr, type, positional, named]) => {
  Curry(op, type, expr, positional, named);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetSymbol, (op, [, sym, path]) => {
  op(vm.Op.GetVariable, sym);
  withPath(op, path);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetLexicalSymbol, (op, [, sym, path]) => {
  op(HighLevelResolutionOpcodes.TemplateLocal, sym, handle => {
    op(vm.Op.ConstantReference, handle);
    withPath(op, path);
  });
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetStrictKeyword, (op, expr) => {
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetFreeAsHelperHead, (op, expr) => {
  op(HighLevelResolutionOpcodes.Local, expr[1], _name => {
    op(HighLevelResolutionOpcodes.Helper, expr, handle => {
      Call(op, handle, null, null);
    });
...
function withPath(op, path) {
  ...
  for (let i = 0; i < path.length; i++) {
    op(vm.Op.GetProperty, path[i]);
  }
}
EXPRESSIONS.add(wireFormat.SexpOpcodes.Undefined, op => PushPrimitiveReference(op, undefined));
EXPRESSIONS.add(wireFormat.SexpOpcodes.HasBlock, (op, [, block]) => {
  expr(op, block);
  op(vm.Op.HasBlock);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.HasBlockParams, (op, [, block]) => {
  expr(op, block);
  op(vm.Op.SpreadBlock);
  op(vm.Op.CompileBlock);
  op(vm.Op.HasBlockParams);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.IfInline, (op, [, condition, truthy, falsy]) => {
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.Not, (op, [, value]) => {
  expr(op, value);
  op(vm.Op.Not);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetDynamicVar, (op, [, expression]) => {
  expr(op, expression);
  op(vm.Op.GetDynamicVar);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.Log, (op, [, positional]) => {
  op(vm.MachineOp.PushFrame);
...
function CompileArgs(op, positional, named, blocks, atNames) {
  ...
  op(vm.Op.PushArgs, names, blockNames, flags);
}
function SimpleArgs(op, positional, named, atNames) {
  if (positional === null && named === null) {
    op(vm.Op.PushEmptyArgs);
    return;
  }
  let count = CompilePositional(op, positional);
  let flags = count << 4;
  if (atNames) flags |= 0b1000;
  let names = util.EMPTY_STRING_ARRAY;
  if (named) {
    names = named[0];
    let val = named[1];
    for (let i = 0; i < val.length; i++) {
  ...
  op(vm.Op.PushArgs, names, util.EMPTY_STRING_ARRAY, flags);
}
...
function YieldBlock(op, to, positional) {
  ...
  op(vm.Op.GetBlock, to);
  op(vm.Op.SpreadBlock);
  op(vm.Op.CompileBlock);
  op(vm.Op.InvokeYield);
  op(vm.Op.PopScope);
  op(vm.MachineOp.PopFrame);
}
...
function InvokeStaticBlockWithStack(op, block, callerCount) {
  ...
  if (count) {
    ...
    for (let i = 0; i < count; i++) {
      op(vm.Op.Dup, vm.$fp, callerCount - i);
      op(vm.Op.SetVariable, parameters[i]);
    }
  }
  PushCompilable(op, block);
  op(vm.Op.CompileBlock);
...
function PushSymbolTable(op, parameters) {
  if (parameters !== null) {
    op(vm.Op.PushSymbolTable, symbolTableOperand({
      parameters
    }));
  } else {
...
function PushCompilable(op, _block) {
  if (_block === null) {
    ...
    op(vm.Op.Constant, blockOperand(_block));
  }
}
...
function SwitchCases(op, bootstrap, matcher) {
  ...
  for (let clause of clauses.slice(0, -1)) {
    op(vm.Op.JumpEq, labelOperand(clause.label), clause.match);
  }

  // Enumerate the clauses in reverse order. Earlier matches will
  // require fewer checks.
  for (let i = clauses.length - 1; i >= 0; i--) {
    let clause = util.unwrap(clauses[i]);
    ...
    if (i !== 0) {
      op(vm.MachineOp.Jump, labelOperand('END'));
    }
  }
...
function Replayable(op, args, body) {
  ...
  op(vm.MachineOp.ReturnTo, labelOperand('ENDINITIAL'));
...
function ReplayableIf(op, args, ifTrue, ifFalse) {
  return Replayable(op, args, () => {
    ...
    op(vm.Op.JumpUnless, labelOperand('ELSE'));
    // Otherwise, execute the code associated with the true branch.
    ifTrue();
    // We're done, so return. In the initial execution, this runs
    // the cleanup code. In the updating VM, it exits the updating
    // routine.
    op(vm.MachineOp.Jump, labelOperand('FINALLY'));
...
function InvokeComponent(op, component, _elementBlock, positional, named, _blocks) {
  ...
  if (compilable) {
    op(vm.Op.PushComponentDefinition, handle);
...
function InvokeDynamicComponent(op, definition, _elementBlock, positional, named, _blocks, atNames, curried) {
  ...
  Replayable(op, () => {
    expr(op, definition);
    op(vm.Op.Dup, vm.$sp, 0);
    return 2;
  }, () => {
    op(vm.Op.JumpUnless, labelOperand('ELSE'));
    if (curried) {
      op(vm.Op.ResolveCurriedComponent);
    } else {
      op(vm.Op.ResolveDynamicComponent, isStrictMode());
    }
    op(vm.Op.PushDynamicComponentInstance);
    InvokeNonStaticComponent(op, {
      capabilities: true,
      elementBlock,
      positional,
...
function InvokeStaticComponent(op, {
  ...
  let bailOut = symbolTable.hasEval || manager.hasCapability(capabilities, vm.InternalComponentCapabilities.prepareArgs);
  if (bailOut) {
    InvokeNonStaticComponent(op, {
      capabilities,
      elementBlock,
  ...
  op(vm.Op.Fetch, vm.$s0);
  op(vm.Op.Dup, vm.$sp, 1);
  op(vm.Op.Load, vm.$s0);
  op(vm.MachineOp.PushFrame);
  ...
  if (manager.hasCapability(capabilities, vm.InternalComponentCapabilities.createArgs)) {
    ...
    if (named !== null) {
      ...
      let val = named[1];
      for (let i = 0; i < val.length; i++) {
        let symbol = symbols.indexOf(util.unwrap(names[i]));
        expr(op, val[i]);
  ...
  op(vm.Op.BeginComponentTransaction, vm.$s0);
  if (manager.hasCapability(capabilities, vm.InternalComponentCapabilities.dynamicScope)) {
    op(vm.Op.PushDynamicScope);
  }
  if (manager.hasCapability(capabilities, vm.InternalComponentCapabilities.createInstance)) {
    op(vm.Op.CreateComponent, blocks.has('default') | 0, vm.$s0);
  }
  op(vm.Op.RegisterComponentDestructor, vm.$s0);
  if (manager.hasCapability(capabilities, vm.InternalComponentCapabilities.createArgs)) {
    op(vm.Op.GetComponentSelf, vm.$s0);
  } else {
    op(vm.Op.GetComponentSelf, vm.$s0, argNames);
  }

  // Setup the new root scope for the component
  op(vm.Op.RootScope, symbols.length + 1, Object.keys(blocks).length > 0 ? 1 : 0);

  // Pop the self reference off the stack and set it to the symbol for `this`
  // in the new scope. This is why all subsequent symbols are increased by one.
  op(vm.Op.SetVariable, 0);

  // Going in reverse, now we pop the args/blocks off the stack, starting with
  // arguments, and assign them to their symbols in the new scope.
  for (const symbol of util.reverse(argSymbols)) {
    ...
    if (symbol === -1) {
      ...
      op(vm.Op.SetVariable, symbol + 1);
    }
  }

  // if any positional params exist, pop them off the stack as well
  if (positional !== null) {
    op(vm.Op.Pop, positional.length);
  }

  // Finish up by popping off and assigning blocks
  for (const symbol of util.reverse(blockSymbols)) {
    op(vm.Op.SetBlock, symbol + 1);
  }
  op(vm.Op.Constant, layoutOperand(layout));
  op(vm.Op.CompileBlock);
  op(vm.MachineOp.InvokeVirtual);
  op(vm.Op.DidRenderLayout, vm.$s0);
  op(vm.MachineOp.PopFrame);
  op(vm.Op.PopScope);
  if (manager.hasCapability(capabilities, vm.InternalComponentCapabilities.dynamicScope)) {
    op(vm.Op.PopDynamicScope);
  }
  op(vm.Op.CommitComponentTransaction);
  op(vm.Op.Load, vm.$s0);
}
function InvokeNonStaticComponent(op, {
  ...
  let bindableAtNames = capabilities === true || manager.hasCapability(capabilities, vm.InternalComponentCapabilities.prepareArgs) || !!(named && named[0].length !== 0);
  ...
  op(vm.Op.PrepareArgs, vm.$s0);
  invokePreparedComponent(op, blocks.has('default'), bindableBlocks, bindableAtNames, () => {
    if (layout) {
      op(vm.Op.PushSymbolTable, symbolTableOperand(layout.symbolTable));
      op(vm.Op.Constant, layoutOperand(layout));
      op(vm.Op.CompileBlock);
    } else {
      op(vm.Op.GetComponentLayout, vm.$s0);
    }
    op(vm.Op.PopulateLayout, vm.$s0);
  });
  op(vm.Op.Load, vm.$s0);
}
function WrappedComponent(op, layout, attrsBlockNumber) {
  op(HighLevelBuilderOpcodes.StartLabels);
  WithSavedRegister(op, vm.$s1, () => {
    op(vm.Op.GetComponentTagName, vm.$s0);
    op(vm.Op.PrimitiveReference);
    op(vm.Op.Dup, vm.$sp, 0);
  });
  op(vm.Op.JumpUnless, labelOperand('BODY'));
  op(vm.Op.Fetch, vm.$s1);
  op(vm.Op.PutComponentOperations);
  op(vm.Op.OpenDynamicElement);
  op(vm.Op.DidCreateElement, vm.$s0);
  YieldBlock(op, attrsBlockNumber, null);
  op(vm.Op.FlushElement);
  op(HighLevelBuilderOpcodes.Label, 'BODY');
  InvokeStaticBlock(op, [layout.block[0], []]);
  op(vm.Op.Fetch, vm.$s1);
  op(vm.Op.JumpUnless, labelOperand('END'));
  op(vm.Op.CloseElement);
  op(HighLevelBuilderOpcodes.Label, 'END');
  op(vm.Op.Load, vm.$s1);
  op(HighLevelBuilderOpcodes.StopLabels);
}
function invokePreparedComponent(op, hasBlock, bindableBlocks, bindableAtNames, populateLayout = null) {
  op(vm.Op.BeginComponentTransaction, vm.$s0);
  op(vm.Op.PushDynamicScope);
  op(vm.Op.CreateComponent, hasBlock | 0, vm.$s0);
  ...
  op(vm.Op.RegisterComponentDestructor, vm.$s0);
  op(vm.Op.GetComponentSelf, vm.$s0);
  op(vm.Op.VirtualRootScope, vm.$s0);
  op(vm.Op.SetVariable, 0);
  op(vm.Op.SetupForEval, vm.$s0);
  if (bindableAtNames) op(vm.Op.SetNamedVariables, vm.$s0);
  if (bindableBlocks) op(vm.Op.SetBlocks, vm.$s0);
  op(vm.Op.Pop, 1);
  op(vm.Op.InvokeComponentLayout, vm.$s0);
  op(vm.Op.DidRenderLayout, vm.$s0);
  op(vm.MachineOp.PopFrame);
  op(vm.Op.PopScope);
  op(vm.Op.PopDynamicScope);
...
function WithSavedRegister(op, register, block) {
  op(vm.Op.Fetch, register);
  block();
  op(vm.Op.Load, register);
}
...
function StdAppend(op, trusting, nonDynamicAppend) {
  SwitchCases(op, () => op(vm.Op.ContentType), when => {
    when(vm.ContentType.String, () => {
      if (trusting) {
        op(vm.Op.AssertSame);
        op(vm.Op.AppendHTML);
      } else {
    ...
    if (typeof nonDynamicAppend === 'number') {
      when(vm.ContentType.Component, () => {
        op(vm.Op.ResolveCurriedComponent);
        op(vm.Op.PushDynamicComponentInstance);
        InvokeBareComponent(op);
      });
      when(vm.ContentType.Helper, () => {
        CallDynamic(op, null, null, () => {
          op(vm.MachineOp.InvokeStatic, nonDynamicAppend);
        });
      });
    } else {
    ...
    when(vm.ContentType.SafeString, () => {
      op(vm.Op.AssertSame);
      op(vm.Op.AppendSafeHTML);
    });
    when(vm.ContentType.Fragment, () => {
      op(vm.Op.AssertSame);
      op(vm.Op.AppendDocumentFragment);
    });
    when(vm.ContentType.Node, () => {
      op(vm.Op.AssertSame);
      op(vm.Op.AppendNode);
    });
  });
...
STATEMENTS.add(wireFormat.SexpOpcodes.Comment, (op, sexp) => op(vm.Op.Comment, sexp[1]));
STATEMENTS.add(wireFormat.SexpOpcodes.CloseElement, op => op(vm.Op.CloseElement));
STATEMENTS.add(wireFormat.SexpOpcodes.FlushElement, op => op(vm.Op.FlushElement));
STATEMENTS.add(wireFormat.SexpOpcodes.Modifier, (op, [, expression, positional, named]) => {
  if (isGetFreeModifier(expression)) {
    op(HighLevelResolutionOpcodes.Modifier, expression, handle => {
      op(vm.MachineOp.PushFrame);
      SimpleArgs(op, positional, named, false);
      op(vm.Op.Modifier, handle);
      op(vm.MachineOp.PopFrame);
    });
  } else {
    expr(op, expression);
    ...
    SimpleArgs(op, positional, named, false);
    op(vm.Op.Dup, vm.$fp, 1);
    op(vm.Op.DynamicModifier);
    op(vm.MachineOp.PopFrame);
  }
});
STATEMENTS.add(wireFormat.SexpOpcodes.StaticAttr, (op, [, name, value, namespace]) => {
  op(vm.Op.StaticAttr, inflateAttrName(name), value, namespace ?? null);
});
STATEMENTS.add(wireFormat.SexpOpcodes.StaticComponentAttr, (op, [, name, value, namespace]) => {
  op(vm.Op.StaticComponentAttr, inflateAttrName(name), value, namespace ?? null);
});
STATEMENTS.add(wireFormat.SexpOpcodes.DynamicAttr, (op, [, name, value, namespace]) => {
  expr(op, value);
  op(vm.Op.DynamicAttr, inflateAttrName(name), false, namespace ?? null);
});
STATEMENTS.add(wireFormat.SexpOpcodes.TrustingDynamicAttr, (op, [, name, value, namespace]) => {
  expr(op, value);
  op(vm.Op.DynamicAttr, inflateAttrName(name), true, namespace ?? null);
});
STATEMENTS.add(wireFormat.SexpOpcodes.ComponentAttr, (op, [, name, value, namespace]) => {
  expr(op, value);
  op(vm.Op.ComponentAttr, inflateAttrName(name), false, namespace ?? null);
});
STATEMENTS.add(wireFormat.SexpOpcodes.TrustingComponentAttr, (op, [, name, value, namespace]) => {
  expr(op, value);
  op(vm.Op.ComponentAttr, inflateAttrName(name), true, namespace ?? null);
});
STATEMENTS.add(wireFormat.SexpOpcodes.OpenElement, (op, [, tag]) => {
  op(vm.Op.OpenElement, inflateTagName(tag));
});
STATEMENTS.add(wireFormat.SexpOpcodes.OpenElementWithSplat, (op, [, tag]) => {
  op(vm.Op.PutComponentOperations);
  op(vm.Op.OpenElement, inflateTagName(tag));
});
STATEMENTS.add(wireFormat.SexpOpcodes.Component, (op, [, expr, elementBlock, named, blocks]) => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.Yield, (op, [, to, params]) => YieldBlock(op, to, params));
STATEMENTS.add(wireFormat.SexpOpcodes.AttrSplat, (op, [, to]) => YieldBlock(op, to, null));
STATEMENTS.add(wireFormat.SexpOpcodes.Debugger, (op, [, debugInfo]) => op(vm.Op.Debugger, debugSymbolsOperand(), debugInfo));
STATEMENTS.add(wireFormat.SexpOpcodes.Append, (op, [, value]) => {
  // Special case for static values
  if (!Array.isArray(value)) {
    op(vm.Op.Text, value === null || value === undefined ? '' : String(value));
    ...
    op(HighLevelResolutionOpcodes.OptionalComponentOrHelper, value, {
      ...
      ifHelper(handle) {
        op(vm.MachineOp.PushFrame);
        Call(op, handle, null, null);
        op(vm.MachineOp.InvokeStatic, stdlibOperand('cautious-non-dynamic-append'));
        op(vm.MachineOp.PopFrame);
      },
      ifValue(handle) {
        op(vm.MachineOp.PushFrame);
  ...
  } else if (value[0] === wireFormat.SexpOpcodes.Call) {
    ...
    op(vm.MachineOp.PushFrame);
    expr(op, value);
    op(vm.MachineOp.InvokeStatic, stdlibOperand('cautious-append'));
    op(vm.MachineOp.PopFrame);
  }
});
STATEMENTS.add(wireFormat.SexpOpcodes.TrustingAppend, (op, [, value]) => {
  if (!Array.isArray(value)) {
    op(vm.Op.Text, value === null || value === undefined ? '' : String(value));
  } else {
    op(vm.MachineOp.PushFrame);
    expr(op, value);
    op(vm.MachineOp.InvokeStatic, stdlibOperand('trusting-append'));
    op(vm.MachineOp.PopFrame);
  }
});
STATEMENTS.add(wireFormat.SexpOpcodes.Block, (op, [, expr, positional, named, blocks]) => {
  if (isGetFreeComponent(expr)) {
    op(HighLevelResolutionOpcodes.Component, expr, component => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.InElement, (op, [, block, guid, destination, insertBefore]) => {
  ReplayableIf(op, () => {
    expr(op, guid);
    ...
    op(vm.Op.PushRemoteElement);
    InvokeStaticBlock(op, block);
    op(vm.Op.PopRemoteElement);
  });
});
STATEMENTS.add(wireFormat.SexpOpcodes.If, (op, [, condition, block, inverse]) => ReplayableIf(op, () => {
  expr(op, condition);
  op(vm.Op.ToBoolean);
  return 1;
}, () => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.Each, (op, [, value, key, block, inverse]) => Replayable(op, () => {
  if (key) {
    expr(op, key);
  } else {
  ...
  op(vm.Op.EnterList, labelOperand('BODY'), labelOperand('ELSE'));
  op(vm.MachineOp.PushFrame);
  op(vm.Op.Dup, vm.$fp, 1);
  op(vm.MachineOp.ReturnTo, labelOperand('ITER'));
  op(HighLevelBuilderOpcodes.Label, 'ITER');
  op(vm.Op.Iterate, labelOperand('BREAK'));
  op(HighLevelBuilderOpcodes.Label, 'BODY');
...
STATEMENTS.add(wireFormat.SexpOpcodes.Let, (op, [, positional, block]) => {
  let count = CompilePositional(op, positional);
  InvokeStaticBlockWithStack(op, block, count);
});
STATEMENTS.add(wireFormat.SexpOpcodes.WithDynamicVars, (op, [, named, block]) => {
  if (named) {
...
STATEMENTS.add(wireFormat.SexpOpcodes.InvokeComponent, (op, [, expr, positional, named, blocks]) => {
  if (isGetFreeComponent(expr)) {
    op(HighLevelResolutionOpcodes.Component, expr, component => {
      InvokeComponent(op, component, null, positional, hashToArgs(named), blocks);
    });
