import { assert, expect, unwrap, debugToString, isSmallInt, Stack, isPresentArray, encodeHandle, EMPTY_STRING_ARRAY, dict, assign, enumerate, encodeImmediate, EMPTY_ARRAY, reverse } from "@glimmer/util";

import { Op, MachineOp, TYPE_SIZE, isMachineOp, MACHINE_MASK, ARG_SHIFT, $v0, $fp, $sp, InternalComponentCapabilities, $s0, $s1, ContentType } from "@glimmer/vm";
...
/* This file is generated by build/debug.js */ let debugCompiler;
 ...
 */ function lookupBuiltInHelper(expr, resolver, meta, constants, type) {
    let {upvars: upvars} = assertResolverInvariants(meta), name = unwrap(upvars[expr[1]]), helper = resolver.lookupBuiltInHelper(name);
    if (DEBUG && null === helper) 
    // Keyword helper did not exist, which means that we're attempting to use a
    // value of some kind that is not in scope
    throw assert(!meta.isStrictMode, "Strict mode errors should already be handled at compile time"), 
    new Error(`Attempted to resolve a ${type} in a strict mode template, but that value was not in scope: ${meta.upvars[expr[1]] ?? "{unknown variable}"}`);
...
class Labels {
    ...
    patch(heap) {
        ...
        for (const {at: at, target: target} of targets) {
            ...
            assert(-1 === heap.getbyaddr(at), "Expected heap to contain a placeholder, but it did not"), 
    ...
    if (function(op) {
      ...
      case HighLevelResolutionOpcodes.Component:
        return function(resolver, constants, meta, [, expr, then]) {
            ...
            let type = expr[0];
            if (DEBUG && expr[0] === SexpOpcodes.GetStrictKeyword) throw assert(!meta.isStrictMode, "Strict mode errors should already be handled at compile time"), 
            new Error(`Attempted to resolve a component in a strict mode template, but that value was not in scope: ${meta.upvars[expr[1]] ?? "{unknown variable}"}`);
            if (type === SexpOpcodes.GetLexicalSymbol) {
                let {scopeValues: scopeValues, owner: owner} = meta, definition = expect(scopeValues, "BUG: scopeValues must exist if template symbol is used")[expr[1]];
                then(constants.component(definition, expect(owner, "BUG: expected owner when resolving component definition")));
            } else {
                let {upvars: upvars, owner: owner} = assertResolverInvariants(meta), name = unwrap(upvars[expr[1]]), definition = resolver.lookupComponent(name, owner);
                if (DEBUG && ("object" != typeof definition || null === definition)) throw assert(!meta.isStrictMode, "Strict mode errors should already be handled at compile time"), 
 ...
 */
        return function(resolver, constants, meta, [, expr, then]) {
            ...
            let type = expr[0];
            if (type === SexpOpcodes.GetLexicalSymbol) {
                let {scopeValues: scopeValues} = meta, definition = expect(scopeValues, "BUG: scopeValues must exist if template symbol is used")[expr[1]];
                then(constants.modifier(definition));
            } else if (type === SexpOpcodes.GetStrictKeyword) {
                let {upvars: upvars} = assertResolverInvariants(meta), name = unwrap(upvars[expr[1]]), modifier = resolver.lookupBuiltInModifier(name);
                if (DEBUG && null === modifier) throw assert(!meta.isStrictMode, "Strict mode errors should already be handled at compile time"), 
                new Error(`Attempted to resolve a modifier in a strict mode template, but it was not in scope: ${name}`);
                then(constants.modifier(modifier, name));
            } else {
                let {upvars: upvars, owner: owner} = assertResolverInvariants(meta), name = unwrap(upvars[expr[1]]), modifier = resolver.lookupModifier(name, owner);
                if (DEBUG && null === modifier) throw assert(!meta.isStrictMode, "Strict mode errors should already be handled at compile time"), 
                new Error(`Attempted to resolve \`${name}\`, which was expected to be a modifier, but nothing was found.`);
                then(constants.modifier(modifier, name));
            }
        }
        /**
 ...
 */ (resolver, constants, meta, op);
      ...
      case HighLevelResolutionOpcodes.Helper:
        return function(resolver, constants, meta, [, expr, then]) {
            ...
            if (type === SexpOpcodes.GetLexicalSymbol) {
                ...
                let {upvars: upvars, owner: owner} = assertResolverInvariants(meta), name = unwrap(upvars[expr[1]]), helper = resolver.lookupHelper(name, owner);
                if (DEBUG && null === helper) throw assert(!meta.isStrictMode, "Strict mode errors should already be handled at compile time"), 
                new Error(`Attempted to resolve \`${name}\`, which was expected to be a helper, but nothing was found.`);
      ...
      case HighLevelResolutionOpcodes.ComponentOrHelper:
        return function(resolver, constants, meta, [, expr, {ifComponent: ifComponent, ifHelper: ifHelper}]) {
            ...
            let type = expr[0];
            if (type === SexpOpcodes.GetLexicalSymbol) {
                let {scopeValues: scopeValues, owner: owner} = meta, definition = expect(scopeValues, "BUG: scopeValues must exist if template symbol is used")[expr[1]], component = constants.component(definition, expect(owner, "BUG: expected owner when resolving component definition"), !0);
                if (null !== component) return void ifComponent(component);
                let helper = constants.helper(definition, null, !0);
                if (DEBUG && null === helper) throw assert(!meta.isStrictMode, "Strict mode errors should already be handled at compile time"), 
                new Error(`Attempted to use a value as either a component or helper, but it did not have a component manager or helper manager associated with it. The value was: ${debugToString(definition)}`);
                ...
                if (null !== definition) ifComponent(constants.resolvedComponent(definition, name)); else {
                    ...
                    if (DEBUG && null === helper) throw assert(!meta.isStrictMode, "Strict mode errors should already be handled at compile time"), 
                    new Error(`Attempted to resolve \`${name}\`, which was expected to be a component or helper, but nothing was found.`);
 ...
 */ (resolver, constants, meta, op);
      ...
      case HighLevelResolutionOpcodes.Local:
        {
            let freeVar = op[1], name = expect(meta.upvars, "BUG: attempted to resolve value but no upvars found")[freeVar];
      ...
      case HighLevelResolutionOpcodes.TemplateLocal:
        {
            let [, valueIndex, then] = op, value = expect(meta.scopeValues, "BUG: Attempted to get a template local, but template does not have any")[valueIndex];
...
class EncoderImpl {
    ...
    commit(size) {
        ...
        return this.heap.pushMachine(MachineOp.Return), this.heap.finishMalloc(handle, size), 
        isPresentArray(this.errors) ? {
            errors: this.errors,
            handle: handle
        } : handle;
    }
        ...
        if ("object" == typeof operand && null !== operand) {
            ...
            switch (operand.type) {
              ...
              case HighLevelOperands.Block:
                return encodeHandle(constants.value((block = operand.value, containing = this.meta, 
                new CompilableTemplateImpl(block[0], containing, {
 ...
 */ function PushPrimitive(op, primitive) {
    let p = primitive;
    var value;
    "number" == typeof p && (p = isSmallInt(p) ? encodeImmediate(p) : (assert(!isSmallInt(value = p), "Attempted to make a operand for an int that was not a small int, you should encode this as an immediate"), 
    {
 ...
 */ function Call(op, handle, positional, named) {
    op(MachineOp.PushFrame), SimpleArgs(op, positional, named, !1), op(Op.Helper, handle), 
    op(MachineOp.PopFrame), op(Op.Fetch, $v0);
}
 ...
 */ function CallDynamic(op, positional, named, append) {
    op(MachineOp.PushFrame), SimpleArgs(op, positional, named, !1), op(Op.Dup, $fp, 1), 
    op(Op.DynamicHelper), append ? (op(Op.Fetch, $v0), append(), op(MachineOp.PopFrame), 
    op(Op.Pop, 1)) : (op(MachineOp.PopFrame), op(Op.Pop, 1), op(Op.Fetch, $v0));
}
 ...
 */ function Curry(op, type, definition, positional, named) {
    op(MachineOp.PushFrame), SimpleArgs(op, positional, named, !1), op(Op.CaptureArgs), 
    expr(op, definition), op(Op.Curry, type, isStrictMode()), op(MachineOp.PopFrame), 
    op(Op.Fetch, $v0);
}
...
class Compilers {
    ...
    compile(op, sexp) {
        let name = sexp[0], index = unwrap(this.names[name]), func = this.funcs[index];
        assert(!!func, `expected an implementation for ${sexp[0]}`), func(op, sexp);
    }
}
...
function evalSymbols(layout) {
    let {block: block} = layout, [, symbols, hasEval] = block;
 ...
 */ function YieldBlock(op, to, positional) {
    SimpleArgs(op, positional, null, !0), op(Op.GetBlock, to), op(Op.SpreadBlock), op(Op.CompileBlock), 
    op(Op.InvokeYield), op(Op.PopScope), op(MachineOp.PopFrame);
}
 ...
 */ function InvokeStaticBlock(op, block) {
    op(MachineOp.PushFrame), PushCompilable(op, block), op(Op.CompileBlock), op(MachineOp.InvokeVirtual), 
 ...
 */ function InvokeStaticBlockWithStack(op, block, callerCount) {
    ...
    if (0 !== count) {
        ...
        PushCompilable(op, block), op(Op.CompileBlock), op(MachineOp.InvokeVirtual), count && op(Op.PopScope), 
        op(MachineOp.PopFrame);
...
function SwitchCases(op, bootstrap, matcher) {
    ...
    op(HighLevelBuilderOpcodes.Label, "END"), op(HighLevelBuilderOpcodes.StopLabels), 
    op(Op.Exit);
}
...
function InvokeComponent(op, component, _elementBlock, positional, named, _blocks) {
    let {compilable: compilable, capabilities: capabilities, handle: handle} = component, elementBlock = _elementBlock ? [ _elementBlock, [] ] : null, blocks = Array.isArray(_blocks) || null === _blocks ? namedBlocks(_blocks) : _blocks;
    compilable ? (op(Op.PushComponentDefinition, handle), function(op, {capabilities: capabilities, layout: layout, elementBlock: elementBlock, positional: positional, named: named, blocks: blocks}) {
        ...
        // or not an argument is used, so we have to give access to all of them.
            ...
            for (let i = 0; i < val.length; i++) {
                let name = unwrap(names[i]), symbol = symbols.indexOf(name);
                -1 !== symbol && (expr(op, val[i]), argSymbols.push(symbol), argNames.push(name));
            }
        }
        op(Op.BeginComponentTransaction, $s0), hasCapability(capabilities, InternalComponentCapabilities.dynamicScope) && op(Op.PushDynamicScope), 
        hasCapability(capabilities, InternalComponentCapabilities.createInstance) && op(Op.CreateComponent, 0 | blocks.has("default"), $s0), 
        op(Op.RegisterComponentDestructor, $s0), hasCapability(capabilities, InternalComponentCapabilities.createArgs) ? op(Op.GetComponentSelf, $s0) : op(Op.GetComponentSelf, $s0, argNames), 
        ...
        op(Op.Constant, layoutOperand(layout)), op(Op.CompileBlock), op(MachineOp.InvokeVirtual), 
        op(Op.DidRenderLayout, $s0), op(MachineOp.PopFrame), op(Op.PopScope), hasCapability(capabilities, InternalComponentCapabilities.dynamicScope) && op(Op.PopDynamicScope), 
        op(Op.CommitComponentTransaction), op(Op.Load, $s0);
    }(op, {
        capabilities: capabilities,
        layout: compilable,
        elementBlock: elementBlock,
...
function InvokeDynamicComponent(op, definition, _elementBlock, positional, named, _blocks, atNames, curried) {
    let elementBlock = _elementBlock ? [ _elementBlock, [] ] : null, blocks = Array.isArray(_blocks) || null === _blocks ? namedBlocks(_blocks) : _blocks;
    Replayable(op, (() => (expr(op, definition), op(Op.Dup, $sp, 0), 2)), (() => {
        op(Op.JumpUnless, labelOperand("ELSE")), curried ? op(Op.ResolveCurriedComponent) : op(Op.ResolveDynamicComponent, isStrictMode()), 
        op(Op.PushDynamicComponentInstance), InvokeNonStaticComponent(op, {
            capabilities: !0,
            elementBlock: elementBlock,
            positional: positional,
            named: named,
...
function InvokeNonStaticComponent(op, {capabilities: capabilities, elementBlock: elementBlock, positional: positional, named: named, atNames: atNames, blocks: namedBlocks, layout: layout}) {
    ...
    op(Op.Fetch, $s0), op(Op.Dup, $sp, 1), op(Op.Load, $s0), op(MachineOp.PushFrame), 
    function(op, positional, named, blocks, atNames) {
        ...
        layout ? (op(Op.PushSymbolTable, symbolTableOperand(layout.symbolTable)), op(Op.Constant, layoutOperand(layout)), 
        op(Op.CompileBlock)) : op(Op.GetComponentLayout, $s0), op(Op.PopulateLayout, $s0);
    })), op(Op.Load, $s0);
}
...
function invokePreparedComponent(op, hasBlock, bindableBlocks, bindableAtNames, populateLayout = null) {
    ...
    // and do nothing here.
    populateLayout && populateLayout(), op(Op.RegisterComponentDestructor, $s0), op(Op.GetComponentSelf, $s0), 
    op(Op.VirtualRootScope, $s0), op(Op.SetVariable, 0), op(Op.SetupForEval, $s0), bindableAtNames && op(Op.SetNamedVariables, $s0), 
    bindableBlocks && op(Op.SetBlocks, $s0), op(Op.Pop, 1), op(Op.InvokeComponentLayout, $s0), 
    op(Op.DidRenderLayout, $s0), op(MachineOp.PopFrame), op(Op.PopScope), op(Op.PopDynamicScope), 
    ...
    SwitchCases(op, (() => op(Op.ContentType)), (when => {
        when(ContentType.String, (() => {
            ...
            op(Op.ResolveCurriedComponent), op(Op.PushDynamicComponentInstance), function(op) {
                op(Op.Fetch, $s0), op(Op.Dup, $sp, 1), op(Op.Load, $s0), op(MachineOp.PushFrame), 
                op(Op.PushEmptyArgs), op(Op.PrepareArgs, $s0), invokePreparedComponent(op, !1, !1, !0, (() => {
...
EXPRESSIONS.add(SexpOpcodes.HasBlock, ((op, [, block]) => {
    ...
    op(MachineOp.PushFrame), SimpleArgs(op, positional, null, !1), op(Op.Log), op(MachineOp.PopFrame), 
    op(Op.Fetch, $v0);
}));
...
function build(program, builder) {
    let {constants: constants, heap: heap, resolver: resolver} = program, encoder = new EncoderImpl(heap, STDLIB_META);
    builder((function(...op) {
...
class CompileTimeCompilationContextImpl {
    ...
    heap;
    stdlib;
    constructor({constants: constants, heap: heap}, resolver, createOp) {
        this.resolver = resolver, this.createOp = createOp, this.constants = constants, 
        this.heap = heap, this.stdlib = compileStd(this);
    }
}
...
STATEMENTS.add(SexpOpcodes.Comment, ((op, sexp) => op(Op.Comment, sexp[1]))), STATEMENTS.add(SexpOpcodes.CloseElement, (op => op(Op.CloseElement))), 
STATEMENTS.add(SexpOpcodes.FlushElement, (op => op(Op.FlushElement))), STATEMENTS.add(SexpOpcodes.Modifier, ((op, [, expression, positional, named]) => {
    isGetFreeModifier(expression) ? op(HighLevelResolutionOpcodes.Modifier, expression, (handle => {
        op(MachineOp.PushFrame), SimpleArgs(op, positional, named, !1), op(Op.Modifier, handle), 
        op(MachineOp.PopFrame);
    })) : (expr(op, expression), op(MachineOp.PushFrame), SimpleArgs(op, positional, named, !1), 
    op(Op.Dup, $fp, 1), op(Op.DynamicModifier), op(MachineOp.PopFrame));
...
STATEMENTS.add(SexpOpcodes.Debugger, ((op, [, debugInfo]) => op(Op.Debugger, {
    type: HighLevelOperands.DebugSymbols,
    value: void 0
}, debugInfo))), STATEMENTS.add(SexpOpcodes.Append, ((op, [, value]) => {
    // Special case for static values
    if (Array.isArray(value)) if (isGetFreeComponentOrHelper(value)) op(HighLevelResolutionOpcodes.OptionalComponentOrHelper, value, {
        ifComponent(component) {
            InvokeComponent(op, component, null, null, null, null);
        },
        ifHelper(handle) {
            op(MachineOp.PushFrame), Call(op, handle, null, null), op(MachineOp.InvokeStatic, stdlibOperand("cautious-non-dynamic-append")), 
            op(MachineOp.PopFrame);
        },
        ifValue(handle) {
            op(MachineOp.PushFrame), op(Op.ConstantReference, handle), op(MachineOp.InvokeStatic, stdlibOperand("cautious-non-dynamic-append")), 
            op(MachineOp.PopFrame);
        }
        ...
        isGetFreeComponentOrHelper(expression) ? op(HighLevelResolutionOpcodes.ComponentOrHelper, expression, {
            ...
            ifHelper(handle) {
                op(MachineOp.PushFrame), Call(op, handle, positional, named), op(MachineOp.InvokeStatic, stdlibOperand("cautious-non-dynamic-append")), 
                op(MachineOp.PopFrame);
            }
        }) : SwitchCases(op, (() => {
    ...
    } else op(MachineOp.PushFrame), expr(op, value), op(MachineOp.InvokeStatic, stdlibOperand("cautious-append")), 
    op(MachineOp.PopFrame); else op(Op.Text, null == value ? "" : String(value));
})), STATEMENTS.add(SexpOpcodes.TrustingAppend, ((op, [, value]) => {
    Array.isArray(value) ? (op(MachineOp.PushFrame), expr(op, value), op(MachineOp.InvokeStatic, stdlibOperand("trusting-append")), 
    op(MachineOp.PopFrame)) : op(Op.Text, null == value ? "" : String(value));
    ...
    expr(op, destination), op(Op.Dup, $sp, 0), 4)), (() => {
...
expr(op, value), 2)), (() => {
    op(Op.EnterList, labelOperand("BODY"), labelOperand("ELSE")), op(MachineOp.PushFrame), 
    op(Op.Dup, $fp, 1), op(MachineOp.ReturnTo, labelOperand("ITER")), op(HighLevelBuilderOpcodes.Label, "ITER"), 
    op(Op.Iterate, labelOperand("BREAK")), op(HighLevelBuilderOpcodes.Label, "BODY"), 
    InvokeStaticBlockWithStack(op, block, 2), op(Op.Pop, 2), op(MachineOp.Jump, labelOperand("FINALLY")), 
    op(HighLevelBuilderOpcodes.Label, "BREAK"), op(MachineOp.PopFrame), op(Op.ExitList), 
    op(MachineOp.Jump, labelOperand("FINALLY")), op(HighLevelBuilderOpcodes.Label, "ELSE"), 
    inverse && InvokeStaticBlock(op, inverse);
...
function compileStatements(statements, meta, syntaxContext) {
    let sCompiler = STATEMENTS, context = templateCompilationContext(syntaxContext, meta), {encoder: encoder, program: {constants: constants, resolver: resolver}} = context;
    function pushOp(...op) {
...
class WrappedBuilder {
    ...
    compile(syntax) {
        ...
        let m = meta(this.layout), context = templateCompilationContext(syntax, m), {encoder: encoder, program: {constants: constants, resolver: resolver}} = context;
        var op, layout, attrsBlockNumber;
        op = function(...op) {
            encodeOp(encoder, constants, resolver, m, op);
        }, layout = this.layout, attrsBlockNumber = this.attrsBlockNumber, op(HighLevelBuilderOpcodes.StartLabels), 
        function(op, register, block) {
            op(Op.Fetch, register), block(), op(Op.Load, register);
        }(op, $s1, (() => {
            op(Op.GetComponentTagName, $s0), op(Op.PrimitiveReference), op(Op.Dup, $sp, 0);
        })), op(Op.JumpUnless, labelOperand("BODY")), op(Op.Fetch, $s1), op(Op.PutComponentOperations), 
        op(Op.OpenDynamicElement), op(Op.DidCreateElement, $s0), YieldBlock(op, attrsBlockNumber, null), 
        op(Op.FlushElement), op(HighLevelBuilderOpcodes.Label, "BODY"), InvokeStaticBlock(op, [ layout.block[0], [] ]), 
        op(Op.Fetch, $s1), op(Op.JumpUnless, labelOperand("END")), op(Op.CloseElement), 
        op(HighLevelBuilderOpcodes.Label, "END"), op(Op.Load, $s1), op(HighLevelBuilderOpcodes.StopLabels);
