...
class NewElementBuilder {
  ...
  popRemoteElement() {
    ...
    util.assert(block instanceof RemoteLiveBlock, '[BUG] expecting a RemoteLiveBlock');
    this.popElement();
    return block;
  }
...
APPEND_OPCODES.add(vm.Op.OpenDynamicElement, vm => {
  let tagName = reference.valueForRef(vm.stack.pop());
...
APPEND_OPCODES.add(vm.Op.PushRemoteElement, vm => {
  ...
  let element = reference.valueForRef(elementRef);
  let insertBefore = reference.valueForRef(insertBeforeRef);
  ...
  if (vm.env.debugRenderTree !== undefined) {
    ...
    destroyable.registerDestructor(block, () => {
...
APPEND_OPCODES.add(vm.Op.FlushElement, vm$1 => {
  let operations = vm$1.fetchValue(vm.$t0);
  let modifiers = null;
  if (operations) {
...
APPEND_OPCODES.add(vm.Op.Modifier, (vm$1, {
  ...
  let state = manager.create(owner, util.expect(constructing, 'BUG: ElementModifier could not find the element it applies to'), definition.state, capturedArgs);
  let instance = {
    manager,
    state,
    definition
  };
  let operations = util.expect(vm$1.fetchValue(vm.$t0), 'BUG: ElementModifier could not find operations to append to');
  operations.addModifier(vm$1, instance, capturedArgs);
...
APPEND_OPCODES.add(vm.Op.DynamicModifier, vm$1 => {
  ...
  if (instance !== undefined) {
    let operations = util.expect(vm$1.fetchValue(vm.$t0), 'BUG: ElementModifier could not find operations to append to');
    operations.addModifier(vm$1, instance, args);
...
APPEND_OPCODES.add(vm.Op.ResolveDynamicComponent, (vm$1, {
  ...
  let component = reference.valueForRef(stack.pop());
...
class ComponentElementOperations {
  ...
  addModifier(vm, modifier, capturedArgs) {
    ...
    if (vm.env.debugRenderTree !== undefined) {
      ...
      util.assert(constructing, `Expected a constructing element in addModifier`);
      ...
      destroyable.registerDestructor(state, () => {
...
function reifyPositionalDebug(positional) {
  return positional.map(p => {
    try {
      return reference.valueForRef(p);
    } catch (e) {
...
APPEND_OPCODES.add(vm.Op.Helper, (vm$1, {
  ...
  let stack = vm$1.stack;
  let helper = vm$1[CONSTANTS].getValue(handle);
  let args = stack.pop();
...
APPEND_OPCODES.add(vm.Op.AppendSafeHTML, vm => {
  let reference$1 = vm.stack.pop();
  let rawValue = reference.valueForRef(reference$1).toHTML();
...
class OnModifierState {
  ...
  updateListener() {
    ...
    util.assert(args.positional[0], 'You must pass a valid DOM event name as the first argument to the `on` modifier');
    let eventName = reference.valueForRef(args.positional[0]);
    util.assert(args.positional[1], 'You must pass a function as the second argument to the `on` modifier');
    let userProvidedCallback = reference.valueForRef(args.positional[1]);
    ...
    if (env.DEBUG) {
      ...
      if (_once) {
        once = reference.valueForRef(_once);
      }
      if (_passive) {
        passive = reference.valueForRef(_passive);
      }
      if (_capture) {
        capture = reference.valueForRef(_capture);
      }
    }
