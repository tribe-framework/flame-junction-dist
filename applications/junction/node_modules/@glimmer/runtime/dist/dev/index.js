import { Op, $pc, CurriedTypes, $t0, $t1, InternalComponentCapabilities, $sp, $v0, ContentType, CurriedType, $ra, $fp, MachineOp, isLowLevelRegister, $s1, $s0 } from "@glimmer/vm";

import { UNDEFINED_REFERENCE, createComputeRef, valueForRef, createConstRef, createPrimitiveRef, isConstRef, NULL_REFERENCE, TRUE_REFERENCE, FALSE_REFERENCE, createDebugAliasRef, childRefFor, createIteratorRef, isInvokableRef, updateRef, createIteratorItemRef } from "@glimmer/reference";

import { assign, unwrap, expect, NS_SVG, castToBrowser, Stack, assert, isObject, decodeHandle, isHandle, decodeImmediate, debugToString, EMPTY_STRING_ARRAY, dict, unwrapTemplate, enumerate, emptyArray, INSERT_BEFORE_END, INSERT_BEFORE_BEGIN, clearElement, INSERT_AFTER_BEGIN, castToSimple, buildUntouchableThis, isDict, unwrapHandle, reverse, COMMENT_NODE } from "@glimmer/util";
...
import { warnIfStyleNotTrusted, toBool, getPath, setPath, assertGlobalContextWasSet } from "@glimmer/global-context";
...
import { beginTrackFrame, endTrackFrame, CONSTANT_TAG, INITIAL, validateTag, consumeTag, valueForTag, CURRENT_TAG, track, updateTag, createCache, getValue, createUpdatableTag, debug, resetTracking } from "@glimmer/validator";

import { RuntimeProgramImpl } from "@glimmer/program";

import { getOwner } from "@glimmer/owner";

/* This file is generated by build/debug.js */ new Array(Op.Size).fill(null), new Array(Op.Size).fill(null);
...
class PartialScopeImpl {
    ...
    partialMap) {
        this.slots = slots, this.owner = owner, this.callerScope = callerScope, this.evalScope = evalScope, 
...
// Glimmer's API.
const INNER_VM = Symbol("INNER_VM"), DESTROYABLE_STACK = Symbol("DESTROYABLE_STACK"), STACKS = Symbol("STACKS"), REGISTERS = Symbol("REGISTERS"), HEAP = Symbol("HEAP"), CONSTANTS = Symbol("CONSTANTS"), ARGS$1 = Symbol("ARGS");
...
}, badProtocols = [ "javascript:", "vbscript:" ], badTags = [ "A", "BODY", "LINK", "IMG", "IFRAME", "BASE", "FORM" ], badTagsForDataURI = [ "EMBED" ], badAttributes = [ "href", "src", "background", "action" ], badAttributesForDataURI = [ "src" ];
...
class InputValueDynamicAttribute extends DefaultDynamicProperty {
    ...
    update(value) {
        const input = castToBrowser(this.attribute.element, [ "input", "textarea" ]), currentValue = input.value, normalizedValue = normalizeStringValue(value);
...
class NewElementBuilder {
    ...
    constructor(env, parentNode, nextSibling) {
        this.pushElement(parentNode, nextSibling), this.env = env, this.dom = env.getAppendOperations(), 
    ...
    flushElement(modifiers) {
        let parent = this.element, element = expect(this.constructing, "flushElement should only be called when constructing an element");
    ...
    popRemoteElement() {
        ...
        return assert(block instanceof RemoteLiveBlock, "[BUG] expecting a RemoteLiveBlock"), 
        this.popElement(), block;
    }
...
class LiveBlockList {
    ...
    openElement(_element) {
        assert(!1, "Cannot openElement directly inside a block list");
    }
    closeElement() {
        assert(!1, "Cannot closeElement directly inside a block list");
    }
    didAppendNode(_node) {
        assert(!1, "Cannot create a new node directly inside a block list");
    }
...
const APPEND_OPCODES = new class {
    ...
    evaluate(vm, opcode, type) {
        ...
        operation.syscall ? (assert(!opcode.isMachine, `BUG: Mismatch between operation.syscall (${operation.syscall}) and opcode.isMachine (${opcode.isMachine}) for ${opcode.type}`), 
        operation.evaluate(vm, opcode)) : (assert(opcode.isMachine, `BUG: Mismatch between operation.syscall (${operation.syscall}) and opcode.isMachine (${opcode.isMachine}) for ${opcode.type}`), 
...
APPEND_OPCODES.add(Op.ChildScope, (vm => vm.pushChildScope())), APPEND_OPCODES.add(Op.PopScope, (vm => vm.popScope())), 
APPEND_OPCODES.add(Op.PushDynamicScope, (vm => vm.pushDynamicScope())), APPEND_OPCODES.add(Op.PopDynamicScope, (vm => vm.popDynamicScope())), 
APPEND_OPCODES.add(Op.Constant, ((vm, {op1: other}) => {
    ...
    let {stack: stack} = vm, handle = stack.pop(), scope = stack.pop(), table = stack.pop();
    assert(null === table || table && "object" == typeof table && Array.isArray(table.parameters), `Expected top of stack to be Option<BlockSymbolTable>, was ${String(table)}`);
    let args = stack.pop();
    if (null === table) 
    ...
    isConstRef(reference) ? !0 === value && vm.goto(target) : (!0 === value && vm.goto(target), 
    vm.updateWith(new Assert(reference)));
})), APPEND_OPCODES.add(Op.JumpUnless, ((vm, {op1: target}) => {
    let reference = vm.stack.pop(), value = Boolean(valueForRef(reference));
    isConstRef(reference) ? !1 === value && vm.goto(target) : (!1 === value && vm.goto(target), 
    vm.updateWith(new Assert(reference)));
...
APPEND_OPCODES.add(Op.Text, ((vm, {op1: text}) => {
    ...
    if (!1 === vm.env.isInteractive) return;
    let {stack: stack} = vm, ref = stack.pop(), args = stack.pop().capture(), {positional: outerPositional, named: outerNamed} = args, {constructing: constructing} = vm.elements(), initialOwner = vm.getOwner(), instanceRef = createComputeRef((() => {
    ...
    return void 0 !== instance && (expect(vm.fetchValue($t0), "BUG: ElementModifier could not find operations to append to").addModifier(vm, instance, args), 
...
class UpdateDynamicModifierOpcode {
    ...
    evaluate(vm) {
        ...
        } else null === tag || validateTag(tag, lastUpdated) || (vm.env.scheduleUpdateModifier(instance), 
        this.lastUpdated = valueForTag(tag));
...
APPEND_OPCODES.add(Op.StaticAttr, ((vm, {op1: _name, op2: _value, op3: _namespace}) => {
    ...
    let name = vm[CONSTANTS].getValue(_name), trusting = vm[CONSTANTS].getValue(_trusting), reference = vm.stack.pop(), value = valueForRef(reference), namespace = _namespace ? vm[CONSTANTS].getValue(_namespace) : null, attribute = vm.elements().setDynamicAttribute(name, value, trusting, namespace);
 ...
 */ APPEND_OPCODES.add(Op.PushComponentDefinition, ((vm, {op1: handle}) => {
    ...
    let {manager: manager, capabilities: capabilities} = definition, instance = {
        definition: definition,
        manager: manager,
    ...
    let stack = vm.stack, instance = vm.fetchValue(_state), args = stack.pop(), {definition: definition} = instance;
    if (isCurriedType(definition, CurriedTypes.Component)) {
        ...
        void 0 !== named && args.named.merge(assign({}, ...named)), void 0 !== positional && (args.realloc(positional.length), 
        args.positional.prepend(positional));
        let {manager: manager} = definition;
        assert(null === instance.manager, "component instance manager should not be populated yet"), 
        assert(null === instance.capabilities, "component instance manager should not be populated yet"), 
    ...
    let instance = vm.fetchValue(_state), {definition: definition, manager: manager, capabilities: capabilities} = instance;
    ...
    let {manager: manager, state: state, capabilities: capabilities} = vm.fetchValue(_state), d = manager.getDestroyable(state);
    ...
    let name;
    if (DEBUG) {
        let {definition: definition, manager: manager} = vm.fetchValue(_state);
...
class ComponentElementOperations {
    ...
    addModifier(vm, modifier, capturedArgs) {
        if (this.modifiers.push(modifier), void 0 !== vm.env.debugRenderTree) {
            ...
            let {element: element, constructing: constructing} = vm.elements(), name = manager.getDebugName(definition.state), instance = manager.getDebugInstance(state);
...
APPEND_OPCODES.add(Op.DidCreateElement, ((vm, {op1: _state}) => {
    let {definition: definition, state: state} = vm.fetchValue(_state), {manager: manager} = definition, operations = vm.fetchValue($t0);
    ...
    let instance = vm.fetchValue(_state), {definition: definition, state: state} = instance, {manager: manager} = definition, selfRef = manager.getSelf(state);
    if (void 0 !== vm.env.debugRenderTree) {
        let args, moduleName, instance = vm.fetchValue(_state), {definition: definition, manager: manager} = instance;
        ...
        if (null === compilable ? (assert(managerHasCapability(manager, instance.capabilities, InternalComponentCapabilities.dynamicLayout), "BUG: No template was found for this component, and the component did not have the dynamic layout capability"), 
        compilable = manager.getDynamicLayout(state, vm.runtime.resolver), moduleName = null !== compilable ? compilable.moduleName : "__default__.hbs") : moduleName = compilable.moduleName, 
        // For tearing down the debugRenderTree
    ...
    let {definition: definition, state: state} = vm.fetchValue(_state), {manager: manager} = definition, tagName = manager.getTagName(state);
...
APPEND_OPCODES.add(Op.GetComponentLayout, ((vm, {op1: _state}) => {
    let instance = vm.fetchValue(_state), {manager: manager, definition: definition} = instance, {stack: stack} = vm, {compilable: compilable} = definition;
    if (null === compilable) {
        ...
        compilable = manager.getDynamicLayout(instance.state, vm.runtime.resolver), null === compilable && (compilable = managerHasCapability(manager, capabilities, InternalComponentCapabilities.wrapped) ? unwrapTemplate(vm[CONSTANTS].defaultTemplate).asWrappedLayout() : unwrapTemplate(vm[CONSTANTS].defaultTemplate).asLayout());
    }
    ...
    let {stack: stack} = vm, handle = stack.pop(), table = stack.pop(), state = vm.fetchValue(_state);
    ...
    let owner, {table: table, manager: manager, capabilities: capabilities, state: state} = vm.fetchValue(_state);
    ...
    let state = vm.fetchValue(_state), scope = vm.scope(), args = vm.stack.peek(), callerNames = args.named.atNames;
    for (let i = callerNames.length - 1; i >= 0; i--) {
        let atName = unwrap(callerNames[i]), symbol = state.table.symbols.indexOf(atName), value = args.named.get(atName, !0);
    ...
    let state = vm.fetchValue(_state), {blocks: blocks} = vm.stack.peek();
...
APPEND_OPCODES.add(Op.InvokeComponentLayout, ((vm, {op1: _state}) => {
    ...
    let instance = vm.fetchValue(_state), {manager: manager, state: state, capabilities: capabilities} = instance, bounds = vm.elements().popBlock();
    ...
    managerHasCapability(manager, capabilities, InternalComponentCapabilities.createInstance) && (manager.didRenderLayout(state, bounds), 
    vm.env.didCreate(instance), vm.updateWith(new DidUpdateLayoutOpcode(instance, bounds)));
...
class NamedArgumentsImpl {
    ...
    get names() {
        ...
        return names || (names = this._names = this._atNames.map(this.toSyntheticName)), 
        names;
    }
    get atNames() {
    ...
    capture() {
        ...
        for (const [i, name] of enumerate(names)) map[name] = DEBUG ? createDebugAliasRef(`@${name}`, unwrap(references[i])) : unwrap(references[i]);
        return map;
    }
    merge(other) {
...
function resolveHelper(definition, ref) {
    ...
    if (null === managerOrHelper ? helper = null : (helper = "function" == typeof managerOrHelper ? managerOrHelper : managerOrHelper.getHelper(definition), 
    assert(managerOrHelper, "BUG: expected manager or helper")), DEBUG && null === helper) throw new Error(`Expected a dynamic helper definition, but received an object or function that did not have a helper manager associated with it. The dynamic invocation was \`{{${ref.debugLabel}}}\` or \`(${ref.debugLabel})\`, and the incorrect definition is the value at the path \`${ref.debugLabel}\`, which was: ${debugToString(definition)}`);
    return helper;
}

function isUndefinedReference(input) {
    return assert(Array.isArray(input) || input === UNDEFINED_REFERENCE, "a reference other than UNDEFINED_REFERENCE is illegal here"), 
    input === UNDEFINED_REFERENCE;
}

APPEND_OPCODES.add(Op.Curry, ((vm, {op1: type, op2: _isStrict}) => {
    let stack = vm.stack, definition = stack.pop(), capturedArgs = stack.pop(), owner = vm.getOwner(), resolver = vm.runtime.resolver, isStrict = !1;
    DEBUG && (
    ...
    let helperRef, stack = vm.stack, ref = stack.pop(), args = stack.pop().capture(), initialOwner = vm.getOwner(), helperInstanceRef = createComputeRef((() => {
        ...
        if (isCurriedType(definition, CurriedTypes.Helper)) {
            ...
            void 0 !== named && (args.named = assign({}, ...named, args.named)), void 0 !== positional && (args.positional = positional.concat(args.positional)), 
    ...
    let name = vm[CONSTANTS].getValue(_name), ref = vm.scope().getPartialMap()[name];
...
function toContentType(value) {
    ...
    }(value) ? ContentType.String : isCurriedType(value, CurriedType.Component) || hasInternalComponentManager(value) ? ContentType.Component : isCurriedType(value, CurriedType.Helper) || hasInternalHelperManager(value) ? ContentType.Helper : isSafeString(value) ? ContentType.SafeString : function(value) {
...
function toDynamicContentType(value) {
    ...
    if (isCurriedType(value, CurriedType.Component) || hasInternalComponentManager(value)) return ContentType.Component;
    if (DEBUG && !isCurriedType(value, CurriedType.Helper) && !hasInternalHelperManager(value)) throw new Error(`Attempted use a dynamic value as a component or helper, but that value did not have an associated component or helper manager. The value was: ${value}`);
...
APPEND_OPCODES.add(Op.ContentType, (vm => {
    ...
    let reference = vm.stack.pop(), rawValue = valueForRef(reference), value = isEmpty$2(rawValue) ? "" : String(rawValue), node = vm.elements().appendDynamicText(value);
...
class ScopeInspector {
    ...
    constructor(scope, symbols, debugInfo) {
        ...
        for (const slot of debugInfo) {
            let name = unwrap(symbols[slot - 1]), ref = scope.getSymbol(slot);
            this.locals[name] = ref;
        }
    }
    get(path) {
        let ref, {scope: scope, locals: locals} = this, parts = path.split("."), [head, ...tail] = path.split("."), evalScope = scope.getEvalScope();
        return "this" === head ? ref = scope.getSelf() : locals[head] ? ref = unwrap(locals[head]) : 0 === head.indexOf("@") && evalScope[head] ? ref = evalScope[head] : (ref = this.scope.getSelf(), 
        tail = parts), tail.reduce(((r, part) => childRefFor(r, part)), ref);
    }
}

APPEND_OPCODES.add(Op.Debugger, ((vm, {op1: _symbols, op2: _debugInfo}) => {
    let symbols = vm[CONSTANTS].getArray(_symbols), debugInfo = vm[CONSTANTS].getArray(decodeHandle(_debugInfo)), inspector = new ScopeInspector(vm.scope(), symbols, debugInfo);
...
class DOMOperations {
    ...
    createElement(tag, context) {
        ...
        isHTMLIntegrationPoint = !1), isElementInSVGNamespace && !isHTMLIntegrationPoint) {
    ...
    insertHTMLBefore(parent, nextSibling, html) {
        ...
        let last;
        if (null === nextSibling) parent.insertAdjacentHTML(INSERT_BEFORE_END, html), last = expect(parent.lastChild, "bug in insertAdjacentHTML?"); else if (nextSibling instanceof HTMLElement) nextSibling.insertAdjacentHTML("beforebegin", html), 
        last = expect(nextSibling.previousSibling, "bug in insertAdjacentHTML?"); else {
            // Non-element nodes do not support insertAdjacentHTML, so add an
            // element and call it on that element. Then remove the element.
            // This also protects Edge, IE and Firefox w/o the inspector open
            // from merging adjacent text nodes. See ./compat/text-node-merging-fix.ts
            const {uselessElement: uselessElement} = this;
            parent.insertBefore(uselessElement, nextSibling), uselessElement.insertAdjacentHTML(INSERT_BEFORE_BEGIN, html), 
            last = expect(uselessElement.previousSibling, "bug in insertAdjacentHTML?"), parent.removeChild(uselessElement);
        }
        ...
        insertHTMLBefore(parent, nextSibling, html) {
            return "" === html || parent.namespaceURI !== svgNamespace ? super.insertHTMLBefore(parent, nextSibling, html) : function(parent, div, html, reference) {
                ...
                // point are parsed in the HTML namespace
                                if (assert("" !== html, "html cannot be empty"), "FOREIGNOBJECT" === parent.tagName.toUpperCase()) {
...
function applyTextNodeMergingFix(document, DOMClass) {
    return document && function(document) {
        ...
        return mergingTextDiv.appendChild(document.createTextNode("first")), mergingTextDiv.insertAdjacentHTML(INSERT_BEFORE_END, "second"), 
...
class DebugRenderTreeImpl {
    ...
    didRender(state, bounds) {
        if (DEBUG && this.stack.current !== state) throw new Error(`BUG: expecting ${this.stack.current}, got ${state}`);
    ...
    reset() {
        if (0 !== this.stack.size) {
            ...
            let root = expect(this.stack.toArray()[0], "expected root state when resetting render tree"), ref = this.refs.get(root);
...
class EnvironmentImpl {
    ...
    constructor(options, delegate) {
        if (this.delegate = delegate, this.isInteractive = delegate.isInteractive, this.debugRenderTree = this.delegate.enableDebugTooling ? new DebugRenderTreeImpl : void 0, 
    ...
    begin() {
        ...
        this.debugRenderTree?.begin(), this[TRANSACTION] = new TransactionImpl;
    }
    get transaction() {
...
class SimpleArgsProxy {
    ...
    constructor(context, computeArgs = (() => EMPTY_ARGS)) {
        ...
        DEBUG ? (ARGS_CACHES.set(this, argsCache), Object.freeze(this)) : this.argsCache = argsCache;
    }
    get named() {
...
function invokeHelper(context, definition, computeArgs) {
    ...
    if (DEBUG && !internalManager) throw new Error(`Expected a helper definition to be passed as the second parameter to invokeHelper, but no helper manager was found. The definition value that was passed was \`${debugToString(definition)}\`. Did you use setHelperManager to associate a helper manager with this value?`);
...
class OnModifierState {
    ...
    updateListener() {
        ...
        assert(args.positional[0], "You must pass a valid DOM event name as the first argument to the `on` modifier");
        let eventName = valueForRef(args.positional[0]);
        assert(args.positional[1], "You must pass a function as the second argument to the `on` modifier");
...
class LowLevelVM {
    ...
    constructor(stack, heap, program, externs, registers) {
        this.stack = stack, this.heap = heap, this.program = program, this.externs = externs, 
    ...
    setPc(pc) {
        assert("number" == typeof pc && !isNaN(pc), "pc is set to a number"), this.registers[$pc] = pc;
    }
    ...
    call(handle) {
        assert(handle < 4294967295, "Jumping to placeholder address"), this.registers[$ra] = this.registers[$pc], 
    ...
    nextStatement() {
        let {registers: registers, program: program} = this, pc = registers[$pc];
        if (assert("number" == typeof pc, "pc is a number"), -1 === pc) return null;
        // We have to save off the current operations size so that
        // when we do a jump we can calculate the correct offset
        // to where we are going. We can't simply ask for the size
        // in a jump because we have have already incremented the
        // program counter to the next instruction prior to executing.
                let opcode = program.opcode(pc), operationSize = this.currentOpSize = opcode.size;
...
class TryOpcode extends BlockOpcode {
    ...
    handleException() {
        let {state: state, bounds: bounds, runtime: runtime} = this;
        destroyChildren(this);
        let elementStack = NewElementBuilder.resume(runtime.env, bounds), vm = state.resume(runtime, elementStack), updating = [], children = this.children = [], result = vm.execute((vm => {
...
class ListBlockOpcode extends BlockOpcode {
    ...
    evaluate(vm) {
        ...
        if (this.lastIterator !== iterator) {
            ...
            this.sync(iterator), this.parentElement().removeChild(marker), this.marker = null, 
            ...
            if (void 0 !== opcode && opcode.key === key) this.retainItem(opcode, item), currentOpcodeIndex++; else if (itemMap.has(key)) {
                ...
                // The item opcode was seen already, so we should move it.
                    ...
                    // already, and we can safely retain this item's opcode.
                                        !1 === seenUnretained ? (this.retainItem(itemOpcode, item), 
                    currentOpcodeIndex = seenIndex + 1) : (this.moveItem(itemOpcode, item, opcode), 
                    currentOpcodeIndex++);
                }
    ...
    insertItem(item, before) {
        let {opcodeMap: opcodeMap, bounds: bounds, state: state, runtime: runtime, children: children} = this, {key: key} = item, nextSibling = void 0 === before ? this.marker : before.firstNode(), elementStack = NewElementBuilder.forInitialRender(runtime.env, {
...
class VM {
    ...
    loadValue(register, value) {
        switch (isLowLevelRegister(register) && this[INNER_VM].loadRegister(register, value), 
        register) {
          case $s0:
            this.s0 = value;
            break;
   ...
   */
    ...
    /**
   * End of migrated.
   */    constructor(runtime, {pc: pc, scope: scope, dynamicScope: dynamicScope, stack: stack}, elementStack, context) {
        this.runtime = runtime, this.elementStack = elementStack, this.context = context, 
        DEBUG && assertGlobalContextWasSet(), this.resume = initVM(context);
        let evalStack = EvaluationStackImpl.restore(stack);
        assert("number" == typeof pc, "pc is a number"), evalStack[REGISTERS][$pc] = pc, 
        evalStack[REGISTERS][$sp] = stack.length - 1, evalStack[REGISTERS][$fp] = -1, this[HEAP] = this.program.heap, 
        this[CONSTANTS] = this.program.constants, this.elementStack = elementStack, this[STACKS].scope.push(scope), 
        this[STACKS].dynamicScope.push(dynamicScope), this[ARGS$1] = new VMArgumentsImpl, 
    ...
    static initial(runtime, context, {handle: handle, self: self, dynamicScope: dynamicScope, treeBuilder: treeBuilder, numSymbols: numSymbols, owner: owner}) {
        let scope = PartialScopeImpl.root(self, numSymbols, owner), state = vmState(runtime.program.heap.getaddr(handle), scope, dynamicScope), vm = initVM(context)(runtime, state, treeBuilder);
        return vm.pushUpdating(), vm;
    }
    static empty(runtime, {handle: handle, treeBuilder: treeBuilder, dynamicScope: dynamicScope, owner: owner}, context) {
    ...
    commitCacheGroup() {
        let opcodes = this.updating(), guard = expect(this[STACKS].cache.pop(), "VM BUG: Expected a cache group"), tag = endTrackFrame();
        opcodes.push(new EndTrackFrameOpcode(guard)), guard.finalize(tag, opcodes.length);
    }
    enter(args) {
        let state = this.capture(args), block = this.elements().pushUpdatableBlock(), tryOpcode = new TryOpcode(state, this.runtime, block, []);
    ...
    enterItem({key: key, value: value, memo: memo}) {
        ...
        let state = this.capture(2), block = this.elements().pushUpdatableBlock(), opcode = new ListItemOpcode(state, this.runtime, block, key, memoRef, valueRef);
    ...
    enterList(iterableRef, offset) {
        let updating = [], addr = this[INNER_VM].target(offset), state = this.capture(0, addr), list = this.elements().pushBlockList(updating), opcode = new ListBlockOpcode(state, this.runtime, list, updating, iterableRef);
        this[STACKS].list.push(opcode), this.didEnter(opcode);
    }
    didEnter(opcode) {
        this.associateDestroyable(opcode), this[DESTROYABLE_STACK].push(opcode), this.updateWith(opcode), 
    ...
    next() {
        let result, {env: env, elementStack: elementStack} = this, opcode = this[INNER_VM].nextStatement();
...
function renderMain(runtime, context, owner, self, treeBuilder, layout, dynamicScope = new DynamicScopeImpl) {
    let handle = unwrapHandle(layout.compile(context)), numSymbols = layout.symbolTable.symbols.length, vm = VM.initial(runtime, context, {
        self: self,
        dynamicScope: dynamicScope,
...
function renderComponent(runtime, treeBuilder, context, owner, definition, args = {}, dynamicScope = new DynamicScopeImpl) {
    return function(vm, context, owner, definition, args) {
        ...
        const compilable = expect(reified.compilable, "BUG: Expected the root component rendered with renderComponent to have an associated template, set with setComponentTemplate"), invocation = {
            handle: unwrapHandle(compilable.compile(context)),
            symbolTable: compilable.symbolTable
        };
        // Needed for the Op.Main opcode: arguments, component invocation object, and
        // component definition.
        return vm.stack.push(vm[ARGS$1]), vm.stack.push(invocation), vm.stack.push(reified), 
...
class RehydrateBuilder extends NewElementBuilder {
    ...
    constructor(env, parentNode, nextSibling) {
        ...
        assert(node, "Must have opening comment for rehydration."), this.candidate = node;
        ...
        if (0 !== startingBlockOffset) {
            ...
            node.parentNode.insertBefore(newClosingBlock, closingNode.nextSibling), this.candidate = newCandidate, 
    ...
    remove(node) {
        const element = expect(node.parentNode, "cannot remove a detached node"), next = node.nextSibling;
    ...
    __pushRemoteElement(element, cursorId, insertBefore) {
        ...
        if (assert(!marker || marker.parentNode === element, "expected remote element marker's parent node to match remote element"), 
        void 0 === insertBefore) {
