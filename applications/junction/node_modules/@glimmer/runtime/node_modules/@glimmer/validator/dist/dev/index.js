...
if (DEBUG) {
    ...
    const TRANSACTION_STACK = [], TRANSACTION_ENV = {
        debugMessage(obj, keyName) {
            let objName;
            return objName = "function" == typeof obj ? obj.name : "object" == typeof obj && null !== obj ? `(an instance of ${obj.constructor && obj.constructor.name || "(unknown class)"})` : void 0 === obj ? "(an unknown tag)" : String(obj), 
...
//////////
const UPDATABLE_TAG_ID = 1, COMPUTE = Symbol("TAG_COMPUTE");
...
class MonomorphicTagImpl {
    ...
    static updateTag(_tag, _subtag) {
        if (DEBUG && _tag[TYPE] !== UPDATABLE_TAG_ID) throw new Error("Attempted to update a tag that was not updatable");
    ...
    static dirtyTag(tag, disableConsumptionAssertion) {
        if (DEBUG && tag[TYPE] !== UPDATABLE_TAG_ID && 0 !== tag[TYPE]) throw new Error("Attempted to dirty a tag that was not dirtyable");
...
function assertCache(value, fnName) {
    if (DEBUG && ("object" != typeof value || null === value || !(FN in value))) throw new Error(`${fnName}() can only be used on an instance of a cache created with createCache(). Called with: ${String(value)}`);
}
    ...
    return {
        getter: function(self) {
            ...
            // If the field has never been initialized, we should initialize it
                        return consumeTag(tagFor(self, key)), hasInitializer && !values.has(self) ? (value = initializer.call(self), 
            values.set(self, value)) : value = values.get(self), value;
        },
...
const GLIMMER_VALIDATOR_REGISTRATION = Symbol("GLIMMER_VALIDATOR_REGISTRATION"), globalObj = function() {
