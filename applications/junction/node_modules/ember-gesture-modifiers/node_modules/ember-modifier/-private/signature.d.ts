...
declare type SignatureKeys = 'Element' | 'Args';
/** @private */
export declare type ElementFor<S> = 'Element' extends keyof S ? S['Element'] extends Element ? S['Element'] : Element : Element;
declare type DefaultPositional = [];
declare type GetOrElse<T, K, Fallback> = K extends keyof T ? T[K] : Fallback;
/**
 ...
 */
export declare type PositionalArgs<S> = keyof S extends SignatureKeys ? 'Args' extends keyof S ? GetOrElse<S['Args'], 'Positional', DefaultPositional> : DefaultPositional : GetOrElse<S, 'positional', unknown[]>;
declare const Empty: unique symbol;
export interface EmptyObject {
    [Empty]?: true;
}
declare type DefaultNamed = EmptyObject;
/**
 * A convenience type utility, primarily for working with args in the `modify`
 * hook on class-based modifiers. Given a signature `S` it will return the
 ...
 */
export declare type NamedArgs<S> = keyof S extends SignatureKeys ? 'Args' extends keyof S ? GetOrElse<S['Args'], 'Named', DefaultNamed> : DefaultNamed : GetOrElse<S, 'named', Record<string, unknown>>;
/** @private */
