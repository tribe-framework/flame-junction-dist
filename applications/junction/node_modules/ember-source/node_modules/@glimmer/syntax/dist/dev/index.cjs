...
class Matcher {
  ...
  matchFor(left, right) {
    ...
    util.assert(util.isPresentArray(nesteds), `no match defined for (${left}, ${right}) and no AnyMatch defined either`);
    const callback = new WhenList(nesteds).first(right);
    util.assert(callback !== null, `no match defined for (${left}, ${right}) and no AnyMatch defined either`);
    return callback;
  }
    ...
    while (seenChars < sourceLength) {
      ...
      if (seenLines === line - 1) {
        ...
        if (env.DEBUG) {
          ...
          util.assert(roundTrip !== null, `the returned offset failed to round-trip`);
          util.assert(roundTrip.line === line, `the round-tripped line didn't match the original line`);
          util.assert(roundTrip.column === column, `the round-tripped column didn't match the original column`);
        }
...
class SpanList {
  ...
  getRangeOffset(fallback) {
    if (util.isPresentArray(this._span)) {
      let first = util.getFirst(this._span);
      let last = util.getLast(this._span);
...
function loc(span) {
  if (Array.isArray(span)) {
    let first = util.getFirst(span);
    let last = util.getLast(span);
...
function getNodeHandler(visitor, nodeType) {
  if (visitor.Program) {
    if (nodeType === 'Template' && !visitor.Template || nodeType === 'Block' && !visitor.Block) {
      util.deprecate(`The 'Program' visitor node is deprecated. Use 'Template' or 'Block' instead (node was '${nodeType}') `);
...
function buildBlock(path, params, hash, _defaultBlock, _elseBlock = null, loc, openStrip, inverseStrip, closeStrip) {
  ...
  if (_defaultBlock.type === 'Template') {
    util.deprecate(`b.program is deprecated. Use b.blockItself instead.`);
  ...
  if (_elseBlock?.type === 'Template') {
    util.deprecate(`b.program is deprecated. Use b.blockItself instead.`);
    util.assert(_elseBlock.blockParams.length === 0, '{{else}} block cannot have block params');
...
function buildElement(tag, options = {}) {
  ...
  if (typeof tag === 'string') {
    ...
    util.assert(tag.type === 'PathExpression', `Invalid tag type ${tag.type}`);
    path = tag;
  } else if ('path' in tag) {
    util.assert(tag.path.type === 'PathExpression', `Invalid tag type ${tag.path.type}`);
    path = tag.path;
  ...
  if (selfClosing) {
    util.assert(_closeTag === null || _closeTag === undefined, 'Cannot build a self-closing tag with a closeTag source location');
  }
...
function buildHead(original, loc) {
  let [head, ...tail] = util.asPresentArray(original.split('.'));
  let headNode = b.head({
    original: head,
    loc: buildLoc(loc || null)
  });
...
function buildPath(path, loc) {
  ...
  if (typeof path !== 'string') {
    if ('type' in path) {
      ...
      util.assert(path.head.indexOf('.') === -1, `builder.path({ head, tail }) should not be called with a head with dots in it`);
      let {
        head,
        tail
      } = path;
...
function buildProgram(body, blockParams, loc) {
  util.deprecate(`b.program is deprecated. Use b.template or b.blockItself instead.`);
...
function buildLegacyMustache({
  ...
  Object.defineProperty(node, 'escaped', {
    enumerable: false,
    get() {
      util.deprecate(`The escaped property on mustache nodes is deprecated, use trusting instead`);
...
function buildLegacyPath({
  ...
  const node = {
    ...
    set original(value) {
      let [head, ...tail] = util.asPresentArray(value.split('.'));
  ...
  Object.defineProperty(node, 'parts', {
    enumerable: false,
    get() {
      util.deprecate(`The parts property on path nodes is deprecated, use head and tail instead`);
      let parts = util.asPresentArray(this.original.split('.'));
      if (parts[0] === 'this') {
    ...
    set(values) {
      util.deprecate(`The parts property on mustache nodes is deprecated, use head and tail instead`);
      let parts = [...values];
  ...
  Object.defineProperty(node, 'this', {
    enumerable: false,
    get() {
      util.deprecate(`The this property on path nodes is deprecated, use head.type instead`);
  ...
  Object.defineProperty(node, 'data', {
    enumerable: false,
    get() {
      util.deprecate(`The data property on path nodes is deprecated, use head.type instead`);
...
function buildLegacyLiteral({
  ...
  Object.defineProperty(node, 'original', {
    enumerable: false,
    get() {
      util.deprecate(`The original property on literal nodes is deprecated, use value instead`);
      return this.value;
    },
    set(value) {
      util.deprecate(`The original property on literal nodes is deprecated, use value instead`);
...
class Builders {
  ...
  atName({
    ...
    const node = {
      ...
      set name(value) {
        util.assert(value[0] === '@', `call builders.at() with a string that starts with '@'`);
        util.assert(value.indexOf('.') === -1, `builder.at() should not be called with a name with dots in it`);
        _name = value;
      },
      get original() {
        return this.name;
  ...
  var({
    ...
    const node = {
      ...
      set name(value) {
        util.assert(value !== 'this', `You called builders.var() with 'this'. Call builders.this instead`);
        util.assert(value[0] !== '@', `You called builders.var() with '${name}'. Call builders.at('${name}') instead`);
        util.assert(value.indexOf('.') === -1, `builder.var() should not be called with a name with dots in it`);
        _name = value;
      },
      get original() {
        return this.name;
...
class Parser {
  ...
  currentNode = null;
  tokenizer;
  constructor(source, entityParser = new simpleHtmlTokenizer.EntityParser(simpleHtmlTokenizer.HTML5NamedCharRefs), mode = 'precompile') {
    this.source = source;
  ...
  get currentAttr() {
    return util.expect(this.currentAttribute, 'expected attribute');
  }
  get currentTag() {
    let node = this.currentNode;
    util.assert(node && (node.type === 'StartTag' || node.type === 'EndTag'), 'expected tag');
    return node;
  }
  get currentStartTag() {
    let node = this.currentNode;
    util.assert(node && node.type === 'StartTag', 'expected start tag');
    return node;
  }
  get currentEndTag() {
    let node = this.currentNode;
    util.assert(node && node.type === 'EndTag', 'expected end tag');
    return node;
  }
  get currentComment() {
    let node = this.currentNode;
    util.assert(node && node.type === 'CommentStatement', 'expected a comment');
    return node;
  }
  get currentData() {
    let node = this.currentNode;
    util.assert(node && node.type === 'TextNode', 'expected a text node');
    return node;
  }
  acceptNode(node) {
  ...
  currentElement() {
    return util.getLast(util.asPresentArray(this.elementStack));
  }
  sourceForNode(node, endNode) {
    ...
    while (currentLine < lastLine) {
      currentLine++;
      line = util.unwrap(this.lines[currentLine]);
...
class HandlebarsNodeVisitors extends Parser {
  ...
  Program(program, blockParams) {
    ...
    util.assert(Array.isArray(blockParams), '[BUG] Program in parser unexpectedly called without block params');
  ...
  parseProgram(node, program) {
    ...
    if (node !== poppedNode) {
      if (poppedNode?.type === 'ElementNode') {
        ...
        util.assert(poppedNode !== undefined, '[BUG] empty parser elementStack');
        util.assert(false, `[BUG] mismatched parser elementStack node: ${node.type}`);
      }
    }
    return node;
  }
...
function calculateRightStrippedOffsets(original, value) {
  ...
  return {
    lines: lineCount,
    columns: util.unwrap(lines[lineCount]).length
  };
}
...
function acceptCallNodes(compiler, node) {
  ...
  const end = util.isPresentArray(params) ? util.getLast(params).loc : path.loc;
...
class TokenizerEventHandlers extends HandlebarsNodeVisitors {
  ...
  finishStartTag() {
    ...
    util.assert(name !== '', 'tag name cannot be empty');
    util.assert(nameStart !== null, 'nameStart unexpectedly null');
    util.assert(nameEnd !== null, 'nameEnd unexpectedly null');
    let nameLoc = nameStart.until(nameEnd);
    let [head, ...tail] = util.asPresentArray(name.split('.'));
    let path = b.path({
      head: b.head({
        original: head,
  ...
  finishEndTag(isVoid) {
    ...
    if (isVoid) {
      ...
      util.assert(element.closeTag === null, 'element.closeTag unexpectedly present');
    } else {
  ...
  appendToTagName(char) {
    ...
    if (tag.type === 'StartTag') {
      ...
      if (tag.nameStart === null) {
        util.assert(tag.nameEnd === null, 'nameStart and nameEnd must both be null');
  ...
  parsePossibleBlockParams() {
    ...
    util.assert(this.tokenizer.state === ATTRIBUTE_NAME, 'must be in TokenizerState.attributeName');
    ...
    const handlers = {
      PossibleAs: next => {
        util.assert(state.state === 'PossibleAs', 'bug in block params parser');
        if (isSpace(next)) {
          // " as ..."
          state = {
      ...
      BeforeStartPipe: next => {
        util.assert(state.state === 'BeforeStartPipe', 'bug in block params parser');
        if (isSpace(next)) {
          this.tokenizer.consume();
        } else if (next === '|') {
          state = {
      ...
      BeforeBlockParamName: next => {
        util.assert(state.state === 'BeforeBlockParamName', 'bug in block params parser');
        if (isSpace(next)) {
          this.tokenizer.consume();
        } else if (next === '') {
      ...
      BlockParamName: next => {
        util.assert(state.state === 'BlockParamName', 'bug in block params parser');
        if (next === '') {
          // The HTML tokenizer ran out of characters, so we are either
      ...
      AfterEndPipe: next => {
        util.assert(state.state === 'AfterEndPipe', 'bug in block params parser');
        if (isSpace(next)) {
          this.tokenizer.consume();
        } else if (next === '') {
          // The HTML tokenizer ran out of characters, so we are either
      ...
      Error: next => {
        util.assert(state.state === 'Error', 'bug in block params parser');
      ...
      Done: () => {
        util.assert(false, 'This should never be called');
      }
    };
    let next;
    do {
    ...
    util.assert(state.state === 'Done', 'bug in block params parser');
  }
  ...
  assembleConcatenatedValue(parts) {
    ...
    util.assertPresentArray(parts, `the concatenation parts of an element should not be empty`);
    let first = util.getFirst(parts);
    let last = util.getLast(parts);
    return b.concat({
      parts,
  ...
  assembleAttributeValue(parts, isQuoted, isDynamic, span) {
    ...
    } else if (util.isPresentArray(parts)) {
      return parts[0];
    } else {
      return b.text({
        chars: '',
...
class CodemodEntityParser extends simpleHtmlTokenizer.EntityParser {
...
const STRICT_RESOLUTION = {
  resolution: () => wireFormat.SexpOpcodes.GetStrictKeyword,
  serialize: () => 'Strict',
...
class ProgramSymbolTable extends SymbolTable {
  ...
  allocateFree(name, resolution) {
    ...
    if (resolution.resolution() === wireFormat.SexpOpcodes.GetFreeAsComponentHead && resolution.isAngleBracket) {
...
class BlockSymbolTable extends SymbolTable {
  ...
  #get(name) {
    ...
    return slot === -1 ? null : util.unwrap(this.slots[slot]);
  }
  getLocalsMap() {
...
class Builder {
  ...
  at(name, symbol, loc) {
    ...
    util.assert(name[0] === '@', `call builders.at() with a string that starts with '@'`);
  ...
  freeVar({
    ...
    util.assert(name !== 'this', `You called builders.freeVar() with 'this'. Call builders.this instead`);
    util.assert(name[0] !== '@', `You called builders.freeVar() with '${name}'. Call builders.at('${name}') instead`);
  ...
  localVar(name, symbol, isTemplateLocal, loc) {
    util.assert(name !== 'this', `You called builders.var() with 'this'. Call builders.this instead`);
    util.assert(name[0] !== '@', `You called builders.var() with '${name}'. Call builders.at('${name}') instead`);
...
class ExpressionNormalizer {
  ...
  normalize(expr, resolution) {
    switch (expr.type) {
      ...
      case 'PathExpression':
        util.assert(resolution, '[BUG] resolution is required');
...
class ElementNormalizer {
  ...
  ElementNode(element) {
    ...
    let [tagHead, ...rest] = util.asPresentArray(tag.split('.'));
  ...
  attr(m) {
    util.assert(m.name[0] !== '@', 'An attr name must not start with `@`');
  ...
  arg(arg) {
    util.assert(arg.name[0] === '@', 'An arg name must start with `@`');
...
class TemplateChildren extends Children {
  assertTemplate(table) {
    if (util.isPresentArray(this.namedBlocks)) {
...
class ElementChildren extends Children {
  ...
  assertComponent(name, table, hasBlockParams) {
    if (util.isPresentArray(this.namedBlocks) && this.hasSemanticContent) {
...
exports.getVoidTags = getVoidTags;
